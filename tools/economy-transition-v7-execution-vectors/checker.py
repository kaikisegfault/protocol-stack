...
The checker fails in both directions, exactly as version six's execution
checker does: a derived key the file does not carry is a failure, and a recorded key no derivation
reaches is also a failure, so partial coverage cannot report success and the
file cannot hold a claim nothing reproduces.
...
`simulation/`, and once by a live run of the execution model.
...
HEADER = """\
# Economy transition v7 execution vectors.
# Hex values are lowercase and contain no prefix.
#
# Every value here is derived twice: once by
# tools/economy-transition-v7-execution-vectors/expected.py, which imports
# nothing from simulation/, and once by a live run of the execution model.
# A boolean vector may only be true, because its name is the claim.
#
# These vectors record what a version-seven chain *does* with the recovery
# pool. The settlement it does it over is test-vectors/economy-transition-v7.txt
# and the transaction surface is version six's, unchanged.
"""
