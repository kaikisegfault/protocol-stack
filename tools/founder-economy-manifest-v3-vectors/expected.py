...
Version three renames one channel and changes nothing else, and the version-two
table below is hand-converted for the same reason: the model derives version
three's table *from* version two's, so a verifier that read the model would be
comparing that derivation with itself.
...
# units". Version three does not move it; the rename funds nothing.
MAXIMUM_SUPPLY_DISPLAY = 56_993_950_100
MAXIMUM_SUPPLY_ATOMIC = MAXIMUM_SUPPLY_DISPLAY * D
SUPERSEDED_MAXIMUM_SUPPLY_DISPLAY = 56_993_950_100
...
# The 2026-08-19 direction, recorded in ADR 0049 and carried in the
# constitution's direct-mint table as "Initial mini-gamified incentives".
RENAMED_CHANNEL_FROM = "initial_mystery_box_incentives"
RENAMED_CHANNEL_TO = "mini_gamified_incentives"
RENAMED_CHANNEL_INDEX = 9
...
DIRECT_MINT_CHANNELS_DISPLAY: dict[str, int] = {
    ...
    RENAMED_CHANNEL_TO: 12_500_100,
}
DIRECT_MINT_SUBTOTAL_DISPLAY = 15_012_620_100

# The superseded v2 allocation, converted by hand from the accepted
# `founder-economy-manifest-v2` cap table. It is carried here to prove that the
# rename moved an identifier and no amount: the two tables must hold the same
# caps in the same order under exactly one differing key.
SUPERSEDED_CHANNELS_DISPLAY: dict[str, int] = {
    ...
    "founder_referral": 2_500_020_000,
    "hub_verified_user_incentives": 1_250_010_000,
    RENAMED_CHANNEL_FROM: 12_500_100,
}
SUPERSEDED_SCHEMA = "protocol-stack/founder-economy-manifest/v2"
SUPERSEDED_LABEL = "protocol-stack:founder-economy:manifest-v2"

SCHEMA = "protocol-stack/founder-economy-manifest/v3"
LABEL = "protocol-stack:founder-economy:manifest-v3"
...
# The rename changes which identifier it names and not how many it covers.
...
SUPERSEDED_CHANNEL_ORDER: tuple[str, ...] = tuple(SUPERSEDED_CHANNELS_DISPLAY)
...
def renamed(channel_id: str) -> str:
    """Apply the one rename version three makes to a version-two identifier."""
...
def check_constitution_is_self_consistent() -> list[str]:
    ...
    if sum(SUPERSEDED_CHANNELS_DISPLAY.values()) != SUPERSEDED_MAXIMUM_SUPPLY_DISPLAY:
        failures.append("the v2 caps do not sum to the superseded maximum supply")
    # The rename is exactly one substitution: the two orders must be equal once
    # it is applied, and unequal before it, so a second changed identifier or a
    # reordering is a failure here rather than a difference nothing compares.
    if tuple(renamed(channel) for channel in SUPERSEDED_CHANNEL_ORDER) != CHANNEL_ORDER:
        failures.append("v2 and v3 do not agree once the one rename is applied")
    if SUPERSEDED_CHANNEL_ORDER == CHANNEL_ORDER:
        failures.append("v3 carries v2's identifiers unchanged, so nothing was renamed")
    if SUPERSEDED_CHANNEL_ORDER[RENAMED_CHANNEL_INDEX] != RENAMED_CHANNEL_FROM:
        failures.append("the renamed channel is not at the recorded index in v2")
    if CHANNEL_ORDER[RENAMED_CHANNEL_INDEX] != RENAMED_CHANNEL_TO:
        failures.append("the renamed channel is not at the recorded index in v3")
