...
Two things are checked here that no earlier version could check at all: that one
person's identity is one record with a set of addresses under it, and that the
constitution's 1,000-seat-per-human bound is enforced.

The settlement section goes the other way. Version four imports version three's
cap, assignment record, and mint walk rather than copying them, so the record it
writes for the same population must be **byte-identical to the value version
three recorded**. That is checked against `test-vectors/economy-transition-v3.txt`
directly, which is a third source rather than either implementation.
"""
...
def check_registry(check: Checker) -> None:
    ...
    # Removal unlinks and touches nothing else: Carol keeps her identity record
    # and her key, and only the claim that a particular account is hers is gone.
    check.equal(
    ...
    # An identity holding no addresses can still add one, which is the recovery
    # path the whole direction exists for.
    ...
    check.equal("registry.counts_agree", model.counts_agree({scenario.ALICE_IDENTITY: 1}))
    check.equal(
...
def _registry_rejections() -> dict[str, str]:
    ...
    outcomes = {
        ...
        "reregistering_an_account": base.register(
            bytes.fromhex("f2" * 32), scenario.ALICE_KEY, scenario.ALICE_FIRST_ADDRESS, 1
        ),
    ...
    # The address bound, at its boundary.
...
def check_seat_bound(check: Checker) -> None:
    ...
    Version three records that this bound "is not enforced by any transition
    here, because enforcing it requires knowing that two biometric hashes belong
    to one human, which is exactly what the chain cannot see". With one identity
    per person in state, it can be, and this is that rule reaching the chain.
    """
    model = identity.Registry()
    model.register(scenario.ALICE_IDENTITY, scenario.ALICE_KEY, scenario.ALICE_FIRST_ADDRESS, 1)
...
def check_self_referral(check: Checker) -> None:
    ...
    # The case version three cannot see: two accounts, one person.
...
def check_settlement_is_version_three(check: Checker, vector_root: Path) -> None:
    """The imported settlement must reproduce version three's recorded values.

    The cap, the assignment record, and the mint walk are unchanged, so the
    record version four writes for the same population must be byte-identical to
    the one version three recorded. Checking it against that accepted file is a
    third source rather than either implementation.
    """
    ...
    # The referral accrual is re-keyed from an account to a HUB identity, which
    # version three's opaque referrer key accepts without a line of new code —
    # itself evidence that the settlement did not move.
    ...
    check.equal(
        ...
        set(accruals) <= {scenario.REFERRER_IDENTITY, scenario.CAPPED_REFERRER_IDENTITY},
    )
...
def check_state_entries(check: Checker) -> None:
    ...
    check.equal(
        ...
        ",".join(
            c.ENTRY_KINDS[kind] for kind in sorted({key[0] for key in entries})
        ),
    )
