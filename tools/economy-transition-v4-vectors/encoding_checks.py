...
from simulation.economy_transition_v4 import envelope, messages, receipt, scenario
...
def check_envelope(check: Checker) -> None:
    ...
    # Purchase no longer carries an enrollment signature, because HUB
    # registration already did that work, so the protocol's largest transaction
    # is smaller than version three's.
...
def check_compatibility(check: Checker, vector_root: Path) -> None:
    ...
    check.equal(
        ...
        all(label.startswith("protocol-stack:v4:") for label in c.HUB_MESSAGE_LABELS),
    )
...
def check_hub_messages(check: Checker) -> None:
    ...
    # The ecosystem verifier signs exactly one of them, which is the whole
    # architecture: an unavailable verifier stops new people joining and stops
    ...
    # Every message names the identity, so one person's signature cannot be
    # presented as another's even where the remaining fields coincide.
    check.equal(
        "message.every_message_names_the_identity",
        all(
            scenario.ALICE_IDENTITY in value for value in built.values()
        ),
    )
