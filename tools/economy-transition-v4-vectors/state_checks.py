...
def check_state_keys(check: Checker) -> None:
    ...
    # The referral balance moves from an account to a person, which is the one
    # place where losing an address would otherwise still lose value.
    check.equal(
...
def check_trees(check: Checker) -> None:
    ...
    check.agree(
        "state.economy_root_empty", e.merkle([], c.ECONOMY_TREE_PREFIX).hex(), empty.hex()
    )
        ...
        len(
            {
                ...
                for prefix in (
                    ...
                    "protocol-stack:v3:economy",
                    "protocol-stack:v2:economy",
                )
            }
        )
        == 3,
...
def check_predecessor_restatements(check: Checker, vector_root: Path) -> None:
    ...
    A four-way non-collision claim is worthless if any of the three
    ...
    for version, path in ((2, "economy-transition-v2.txt"), (3, "economy-transition-v3.txt")):
...
def check_state_root(check: Checker) -> None:
    """All four roots must differ on identical accounts and an empty economy."""
    four = state.state_root(**COMMON, economy={})
    three = state.predecessor_state_root(3, **COMMON)
    two = state.predecessor_state_root(2, **COMMON)
    one = state.predecessor_state_root(1, **COMMON)
    check.equal("state.root_empty_economy", four)
    check.equal("state.version_three_root_same_accounts", three)
    check.equal("state.version_two_root_same_accounts", two)
    check.equal("state.version_one_root_same_accounts", one)
    for name, value in (("one", one), ("two", two), ("three", three)):
        check.equal(f"state.roots_differ_from_version_{name}", value != four)
    check.equal("state.all_four_roots_are_distinct", len({one, two, three, four}) == 4)
    ...
    check.equal("state.root_tracks_the_economy", populated != four)
...
def check_genesis(check: Checker) -> None:
    ...
    # The same fields under an earlier schema and label are a different chain,
    # which is what makes the four contracts alternative chains.
    ...
    check.equal("genesis.chain_ids_are_distinct", len(identifiers) == 3)
    ...
    check.equal("genesis.writes_no_hub_identity", not any(
...
def check_storage_bounds(check: Checker) -> None:
    ...
    # The registry is bounded per participant rather than at seat capacity,
    # because it serves every participant class. The constitutional minimum of
    # 100 principals holding all seats is the only figure the constitution
    # itself bounds.
    check.equal(
    ...
    check.equal("storage.cycle_assignment_matches_version_three_width", per_cycle == 25_033)
