...
// The one carried entry whose value version nine widens. The two refusals are
// the identities stated over a single entry, so a value that could not have been
// written by any conforming settlement is refused rather than encoded.
std::optional<Bytes> unreferred_pool_value(const UnreferredPool& pool) {
  if (pool.payable_atomic > pool.accrued_atomic) return std::nullopt;
  if (pool.minted_atomic > pool.accrued_atomic - pool.payable_atomic) {
    return std::nullopt;
  }
  Bytes value;
  i::append_u64(value, pool.accrued_atomic);
  i::append_u64(value, pool.payable_atomic);
  i::append_u64(value, pool.minted_atomic);
  return value;
}
...
std::optional<UnreferredPool> decode_unreferred_pool_value(
  ...
  const auto width =
      entry_value_bytes(static_cast<std::uint8_t>(Entry::unreferred_pool));
  if (!width || raw.size() != *width) return std::nullopt;
  const auto accrued = i::read_u64(raw, 0);
  const auto payable = i::read_u64(raw, 8);
  const auto minted = i::read_u64(raw, 16);
  if (!accrued || !payable || !minted) return std::nullopt;
  const UnreferredPool pool{*accrued, *payable, *minted};
  ...
  if (!unreferred_pool_value(pool)) return std::nullopt;
  return pool;
}
