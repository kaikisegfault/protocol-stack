// The economy tree, the version-nine state root, and the one variable-width
...
// The two fields a quiet height changes, written between the head and the tail.
// A stamp outside the accepted range is refused rather than hashed: C1 refuses
// such a block, so no accepted state holds one, and a root over it would be an
// artifact no conforming chain can reach.
std::optional<Hash> state_root_from_frame(const StateRootFrame& frame,
                                          std::uint64_t height,
                                          std::uint64_t timestamp) {
  if (!timestamp_in_range(timestamp)) return std::nullopt;
  Bytes payload;
  payload.reserve(frame.head.size() + 16 + frame.tail.size());
  i::append(payload, frame.head);
  i::append_u64(payload, height);
  i::append_u64(payload, timestamp);
...
std::optional<Hash> state_root(const StateSummary& summary,
  ...
  return state_root_from_frame(*frame, summary.height, summary.timestamp);
}

// An earlier version's root over the same inputs, so that each of the eight
...
// collision implies nothing about another, so version nine must prove eight.
//
// **No predecessor preimage carries a timestamp**, so a version-nine root could
// not equal one even where a label collided. The eight comparisons are recorded
// anyway, because that argument is about the preimage and each label is its own
// claim.
//
...
std::optional<Hash> predecessor_state_root(std::uint16_t version,
                                           ...
                                           std::span<const AccountEntry> accounts,
                                           std::vector<EconomyEntry> economy) {
  if (version < 1 || version > 8) return std::nullopt;
