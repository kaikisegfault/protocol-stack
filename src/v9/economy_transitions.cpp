...
std::optional<Outcome> dispatch(Ledger& ledger, const Envelope& envelope,
  ...
  switch (static_cast<Kind>(envelope.kind)) {
    ...
    // The two version eight adds. Both are scheme 1, so an escrow always
    // resolved; the response's is the one the audited seat's identity owns and
    // the dispute's is whoever relays it and pays.
    case Kind::challenge_response:
      return submit_response(ledger, body, *escrow);
    case Kind::file_dispute:
      return file_dispute(ledger, envelope, body, *escrow, verify);
    ...
    // Version nine's one addition, routed with the other mints because it is
    // one: kind 22 is kind 4 with a different balance.
