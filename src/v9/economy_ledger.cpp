// The version-nine ledger: opening it, moving value inside it, and projecting
...
std::optional<Ledger> open_ledger(const Genesis& genesis) {
  ...
  // here so the dispute transition can check a signature against it. No open
  // challenge and no seat window record is written at genesis: a challenge is
  // issued by a block, and a window record exists only once a seat has lost or
  // had a slot voided. No figure and no claim either, for the same reason.
  ledger.dispute_authority_key = genesis.dispute_authority_key;

  // **The tenth field and the two entries version nine adds.** Window zero's
  // opening height is genesis — height zero is never a block — so writing its
  // month here is the general rule reaching the one height that is a genesis
  // rather than a block. A genesis whose timestamp has no month is one
  // `encode_genesis` would have refused, so this cannot fire for any genesis
  // whose chain identity was derivable above; it is checked because a
  // derivation that silently produced a month for an out-of-range stamp would
  // be the defect this returns `nullopt` for.
  const auto month = month_index(genesis.genesis_timestamp);
  if (!month) return std::nullopt;
  ledger.timestamp = genesis.genesis_timestamp;
  ledger.accumulating_month = *month;
  ledger.window_months.emplace(0, *month);
  return ledger;
}
...
std::vector<EconomyEntry> economy_entries(const Ledger& ledger) {
  ...
  // The one carried entry version nine widens. The encoder refuses a value no
  // conforming settlement could have written, and a refusal here is a projection
  // that cannot be committed rather than a root over an impossible state.
  const auto pool =
      unreferred_pool_value({ledger.pool_accrued, ledger.pool_payable,
                             ledger.pool_minted});
  push(entries, unreferred_pool_key(), pool.value_or(Bytes{}));
  ...
  // Version nine's four, projected from typed fields rather than copied from a
  // raw map. Each encoder refuses a value no transition could have written — a
  // month outside the calendar, a zero figure, a zero claim — and a refusal
  // produces an entry the root's own shape rules then reject, so a field that
  // encoded badly fails at the commitment rather than surviving as a value
  // nothing reads.
  push(entries, settlement_cursor_key(),
       settlement_cursor_value(ledger.accumulating_month).value_or(Bytes{}));
  for (const auto& [window, month] : ledger.window_months) {
    push(entries, window_month_key(window),
         window_month_value(month).value_or(Bytes{}));
  }
  for (const auto& [key, seconds] : ledger.figures) {
    const auto entry_key = monthly_figure_key(key.first, key.second);
    push(entries, entry_key.value_or(Bytes{}),
         monthly_figure_value(seconds).value_or(Bytes{}));
  }
  for (const auto& [seat_id, claim] : ledger.claims) {
    push(entries, monthly_claim_key(seat_id),
         monthly_claim_value(claim).value_or(Bytes{}));
  }
  return entries;
}
