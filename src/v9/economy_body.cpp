...
std::optional<Body> decode_body(std::uint8_t kind,
  ...
  switch (static_cast<Kind>(kind)) {
    ...
    // **Kind 22 shares kind 4's arm and that is the contract rather than a
    // saving.** `economy-transition-v9` defines its body as kind 4's exactly,
    // field for field, so decoding them apart would be two statements of one
    // layout with nothing keeping the two equal.
    case Kind::mint_node:
