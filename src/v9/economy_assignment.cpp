...
//
// **Version eight derives the measurement instead of accepting one**, and
// `derive_schedule` below is the whole of that change. Everything after it is
// version seven's, which is what makes "the carrier changed no settlement" a
...
bool apply_assignment(Ledger& ledger, const Assignment& assignment) {
  ...
  // the only place `payable` rises**, and raising the two together is what makes
  // the first pool identity hold at every height rather than only after a
  // settlement. The bound follows from the one above rather than repeating it:
  // `payable` never exceeds `accrued`, which the encoder also refuses.
  if (assignment.unreferred_accrual > kMaxU64 - ledger.pool_payable) return false;
  ledger.pool_payable += assignment.unreferred_accrual;
...
bool seat_in_span(std::uint64_t activation_height, std::uint64_t cycle_window) {
  const auto first = first_cycle_window(activation_height);
  return first <= cycle_window && cycle_window < first + kIssuanceCyclesPerSeat;
}

std::uint64_t seat_uptime_seconds(const SeatWindowRecord& record) {
  return uptime_seconds(record);
}

std::vector<SeatCycle> derive_schedule(const Ledger& ledger,
                                       std::uint64_t cycle_window) {
  std::vector<SeatCycle> measured;
  // `std::map` is ordered, so the seat table is already in ascending seat order
  // and the schedule inherits it. Every step that walks seats — this one, the
  // issue step, and the prologue's deletion — is in the same order, so a reader
  // comparing them finds one rule rather than three.
  for (const auto& [seat_id, seat] : ledger.seats) {
    // A purchased but unactivated seat has no activation height and is in no
    // window's scope, which is why the activation flag is read rather than the
    ...
    // gate reports; here it reads as the fully credited default, so the
    // derivation stays total and the refusal happens where it can name itself.
    cycle.uptime_seconds =
        seat_uptime_seconds(record ? *record : full_seat_window());
    cycle.in_span = seat_in_span(seat.activation_height, cycle_window);
    measured.push_back(cycle);
  }
  return measured;
}
