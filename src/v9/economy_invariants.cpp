// Every conservation and structural equality a version-nine state must satisfy.
//
...
void check_channel_identities(const Ledger& ledger,
  ...
  // **Version six's referral channel identity is unchanged and is refined
  // rather than replaced.** The pool's outstanding term splits into a balance
  // and a set of claims without changing its total, which the two pool
  // identities above give: `accrued - minted = payable + sum(claim.accrued -
  // claim.minted)`. That is the substance of the whole settlement — value
  // already counted against the channel's cap moves from undistributed to owed
  // to collected, and no step creates or destroys a unit.
...
void check_uptime(const Ledger& ledger,
                  std::vector<std::string_view>& failures) {
  const auto height = ledger.height;
  const auto window = window_of_height(height);
  for (const auto& [key, value] : ledger.uptime) {
    if (key.empty()) {
      failures.push_back("an uptime entry has an empty key");
      continue;
    }
    const auto kind = static_cast<Entry>(key.front());
    const auto parsed = i::read_u64(key, 1);
    if (!parsed || key.size() != 13) {
      failures.push_back("an uptime entry key is not its fixed width");
      continue;
    }
    if (kind == Entry::open_challenge) {
      // 1. An open challenge entry's `state` is `0` or `1`.
      if (!decode_open_challenge_value(value)) {
        failures.push_back("an open challenge state is neither zero nor one");
      }
      // 2. Every retained challenge's height is inside the deadline window, so
      //    the pipeline retains no challenge whose deadline has passed.
      const auto lower =
          height > kResponseDeadlineBlocks ? height - kResponseDeadlineBlocks + 1 : 0;
      if (*parsed > height || *parsed < lower) {
        failures.push_back("an open challenge outlived its response deadline");
      }
      continue;
    }
    if (kind != Entry::seat_window) {
      failures.push_back("an uptime entry is neither kind eighteen nor nineteen");
      continue;
    }
    // 3. Both bitmaps have their upper eight bits clear and `disputed` is a
    //    subset of `credited`. The decoder states both rules, so a value that
    //    decodes has satisfied them and one that does not is the failure.
    const auto record = decode_seat_window_value(value);
    if (!record) {
      failures.push_back("a seat window record does not decode");
      continue;
    }
    // 4. `popcount(disputed)` never exceeds the founder-directed grace
    //    allowance.
    if (static_cast<std::uint32_t>(std::popcount(record->disputed)) >
        kDisputeCapSlotsPerSeat) {
      failures.push_back("a seat window record exceeds the dispute cap");
    }
    // 5. A record exists only for the executing height's window and the one
    //    before it, so exactly two are retained at every height including a
    //    boundary height — which holds because the prologue deletes the due
    //    window's records before anything in that block can write to the window
    //    that just opened.
    if (*parsed > window || *parsed + 1 < window) {
      failures.push_back("a seat window record outlived its retention");
    }
    // 6. The containment theorem restated over the encoded state: a seat
    //    credited for every slot still meets its cycle after any admissible set
    //    of disputes. It is asserted here rather than left to the cap
    //    arithmetic, because it is what makes the interim single dispute
    //    authority safe.
    if (record->credited == kSlotBitmapMask &&
        uptime_seconds(*record) < kActivityThresholdSeconds) {
      failures.push_back("a maximal dispute failed a fully credited seat");
    }
  }
}

// The three version nine adds that are about a ledger rather than an encoding.
//
// **The other five are the decoders' and fire at the projection.** A month above
// the calendar bound, a zero figure, a zero claim, and a claim that minted more
// than it accrued are all values the encoders refuse to write, so a field
// carrying one produces an entry the root's own shape rules reject and the block
// is rejected whole. Stating them again here would be a second opinion about a
// rule that already fails closed.
void check_monthly(const Ledger& ledger, std::vector<std::string_view>& failures) {
  // Invariant 1. Genesis writes the genesis timestamp and C1 refuses any block
  // stamp outside the range, so an out-of-range value here is a defect in this
  // kernel rather than an input.
  if (!timestamp_in_range(ledger.timestamp)) {
    failures.push_back("the state's timestamp is outside the accepted range");
  }

  // Invariant 2, stated over the state rather than over the history that
  // produced it: an entry exists for the open window and, once one has closed,
  // its predecessor, and for no other window. Before the first window-opening
  // height only genesis's entry exists.
  const auto window = window_of_height(ledger.height);
  std::set<std::uint64_t> expected{window};
  if (window >= 1) expected.insert(window - 1);
  std::set<std::uint64_t> held;
  for (const auto& [recorded, month] : ledger.window_months) {
    (void)month;
    held.insert(recorded);
  }
  if (held != expected) {
    failures.push_back("a window month entry outlived its retention");
  }

  // Invariant 6. Exactly one month accumulates at a time and it is the cursor's:
  // a stale figure from another month would be counted into a ranking it does
  // not belong to. This is why the month is in the key at all — the entry is
  // then visibly wrong rather than silently counted.
  for (const auto& [key, seconds] : ledger.figures) {
    (void)seconds;
    if (key.first != ledger.accumulating_month) {
      failures.push_back("a monthly figure belongs to a month that is not open");
      break;
    }
  }

  for (const auto failure : pool_failures(
           {ledger.pool_accrued, ledger.pool_payable, ledger.pool_minted},
           ledger.claims)) {
...
// The six version eight adds, each checked rather than assumed.
//
// They are stated over the *encoded* state rather than over a typed shadow,
// because that is the state the root commits to and the two transitions write.
// A record that will not decode is itself a failure, which is what makes the
// pad rule and the subset rule invariants rather than only decoder rules.
std::vector<std::string_view> uptime_failures(const Ledger& ledger) {
  std::vector<std::string_view> failures;
  check_uptime(ledger, failures);
  // **Version nine's three run at a quiet height too**, and the cost argument
  // that put the uptime six here rather than in the full gate applies to them
  // unchanged: they are bounded by the two window-month entries, the open
  // month's figures, and the claims, none of which a quiet height can change —
  // but a quiet height *does* move the timestamp and the height, which is
  // exactly what invariants 1 and 2 are stated over.
  check_monthly(ledger, failures);
