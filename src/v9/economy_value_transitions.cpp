// The nine value transitions: the two transfers, the four seat transitions, the
// referral mint, the verified-user mint, the refused direct issue, and version
// nine's monthly pool mint.
//
// **Kind 22 is here rather than in version nine's own translation unit, and that
// placement is the argument.** Its body, its scheme, its authority, its
// destination rule, its posture confirmation, its channel and its fixed fee are
// all kind 4's, and what differs is which balance it empties. A reader checking
// that claim reads the two functions side by side; putting them in different
// files would make the claim something to take on trust.
//
...
namespace protocol::v9::internal {
namespace {
...
// award: a mint that can take a chosen amount must record what it took, so a
// per-month award would charge `n` fees to collect `n` months.
//
// **A seat past its own 731 cycles may hold a claim and mints it here.** In
// scope is permanent under ADR 0049's rule 3, so a seat whose distribution has
// finished still competes for the monthly pool and still wins; this transition
// is not gated on span, exactly as kind 4 is not.
std::optional<Outcome> mint_monthly_pool(Ledger& ledger, const Envelope& envelope,
                                         const Body& body, const Octets32& escrow,
                                         const SignatureVerifier& verify) {
  ...
  // **Reachable, and kept even though the claim condition below would catch
  // it.** A seat cannot hold a claim without having been in scope, and cannot
  // have been in scope without activation, so an unactivated seat always has an
  // empty claim — but a refusal that says `NOTHING_TO_MINT` about a seat nobody
  // ever switched on is true and useless.
  if (!seat->second.is_activated) return refused(Result::seat_not_activated);

  const auto& identity = owner_of(ledger, escrow);
  if (seat->second.hub_identity_hash != identity) return refused(Result::unauthorized);
  ...
  const auto claim = ledger.claims.find(body.seat_id);
  if (claim == ledger.claims.end()) return refused(Result::nothing_to_mint);
  if (claim->second.minted_atomic > claim->second.accrued_atomic) return std::nullopt;
  if (claim->second.accrued_atomic == claim->second.minted_atomic) {
    return refused(Result::nothing_to_mint);
  }
  const auto amount = claim->second.accrued_atomic - claim->second.minted_atomic;

  const auto message =
      mint_message(ledger.chain_id, identity,
                   static_cast<std::uint8_t>(Kind::mint_monthly_pool), body.seat_id,
                   destination, envelope.valid_until_height);
  if (const auto refusal = require_confirmation(ledger, destination, amount, identity,
                                                message, body.hub_signature, verify)) {
    return refused(*refusal);
  }
  // **Structurally unreachable and checked rather than assumed.** The units were
  // counted into the referral channel's `outstanding` when they accrued, and
  // this moves them to `issued` without creating any, so the channel identity
  // forbids the overflow this tests for. It is declared because kind 5 declares
  // it, and because an invariant that cannot fail is cheaper to check than to
  // argue about.
  if (!fits_channel(ledger, kReferralChannel, amount)) {
    return refused(Result::channel_cap);
  }
  ...
  if (!credit(ledger, destination, amount)) return std::nullopt;
  claim->second.minted_atomic = claim->second.accrued_atomic;
  if (amount > kMaxU64 - ledger.pool_minted) return std::nullopt;
  ledger.pool_minted += amount;
  if (!issue(ledger, kReferralChannel, amount)) return std::nullopt;
  // The mint charges the fixed fee, like every other mint, and version nine adds
  // no exemption. Version eight's exemption for kind 20 is a founder answer
  // about a cost a machine bears every hour to prove uptime it is paid for;
  // collecting an award is the ordinary shape.
  const auto outcome = charged(ledger, escrow);
  if (!outcome) return std::nullopt;
  return issued(*outcome, amount);
}
...
std::optional<Outcome> dispatch_value(Ledger& ledger, const Envelope& envelope,
  ...
  switch (static_cast<Kind>(envelope.kind)) {
    ...
    case Kind::mint_monthly_pool:
      return mint_monthly_pool(ledger, envelope, body, escrow, verify);
