...
// The two fee-exempt kinds, and they are not symmetric.
//
...
//
// **A challenge response keeps its nonce and gives up only the fee limit.** It
// has an escrow and therefore a nonce sequence, so replay protection is doubled
// rather than replaced: the nonce and the open challenge entry each refuse a
// second submission independently. The fee limit is required to be zero for the
// registration's reason — a second encoding of "not applicable" is the
// non-minimal representation `protocol-primitives-v1` forbids — which is what
// makes `FEE_LIMIT_TOO_LOW` unreachable for this kind rather than merely
// unused.
bool envelope_shape_is_canonical(std::uint8_t kind, std::uint64_t nonce,
                                 std::uint64_t fee_limit) {
  if (kind == static_cast<std::uint8_t>(Kind::hub_register)) {
    return nonce == 0 && fee_limit == 0;
  }
  if (kind == static_cast<std::uint8_t>(Kind::challenge_response)) {
    return fee_limit == 0;
  }
  return true;
}
