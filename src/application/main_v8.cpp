// `protocol-application-v8`: a version-eight ledger, application, and socket in
// one process.
//
// It is version seven's `main_v7.cpp` with the version rebound and **one figure
// moved**: a version-eight genesis is 142 octets rather than 110, because
// `dispute_authority_key` is a ninth field. That figure is read from
// `v8::kGenesisPrefixBytes` rather than written out, so this file states it
// nowhere and the diff against version seven's is the rebinding alone.
//
// The store takes a `Genesis` **struct** rather than the canonical octets,
// so this reads the file and decodes it, and the store re-encodes what it was
// given and stores those octets. A file that would not have been produced by
...
int fail(std::string_view message) {
  std::cerr << "protocol-application-v8: " << message << '\n';
  return 1;
}

// The size check here is an **allocation bound**, not a validity rule: a
// version-eight genesis is exactly `kGenesisPrefixBytes` octets, so nothing
...
//
// **The width is the one figure that moved with the version**, and it moved by
// changing which header declares `kGenesisPrefixBytes`. A version-seven genesis
// file presented to this binary is 32 octets short and is refused here; a file
// of the right width that version eight would not have written is refused by
// `decode_genesis` a few lines later.
std::variant<v8::Genesis, std::string_view> read_genesis(
  ...
  if (error || size != v8::kGenesisPrefixBytes) {
    return std::string_view{
        "genesis file is not the canonical version-eight width"};
  }
  v8::Bytes bytes(static_cast<std::size_t>(size));
  ...
  auto genesis = v8::decode_genesis(bytes);
  if (!genesis) {
    return std::string_view{
        "genesis file is not a canonical version-eight genesis"};
  }
  return *genesis;
}
...
// case.
ps::SQLiteLedgerV8Result open_or_create_ledger(
    const std::filesystem::path& path, const v8::Genesis& genesis) {
  auto opened = ps::open_sqlite_ledger_v8(path, genesis);
  if (!std::holds_alternative<ps::SQLiteLedgerV8Error>(opened.result) ||
      std::get<ps::SQLiteLedgerV8Error>(opened.result) !=
          ps::SQLiteLedgerV8Error::path_not_found) {
    return opened;
  }
  return ps::create_sqlite_ledger_v8(path, genesis);
}
...
int print_genesis_identity(const std::filesystem::path& genesis_path) {
  ...
  if (!std::holds_alternative<v8::Genesis>(genesis)) {
    return fail(std::get<std::string_view>(genesis));
  }
  const auto& value = std::get<v8::Genesis>(genesis);
  const auto identity = v8::chain_id(value);
  auto opened = v8::open_ledger(value);
...
int run_application(int argc, char** argv) {
  ...
  if (argc != 4) {
    return fail(
        "usage: protocol-application-v8 <absolute-database> "
        "<absolute-genesis> <absolute-socket> | "
        "protocol-application-v8 --genesis-identity <absolute-genesis>");
  }
  ...
  auto ledger =
      open_or_create_ledger(database_path, std::get<v8::Genesis>(genesis));
  if (!std::holds_alternative<ps::SQLiteLedgerV8>(ledger.result)) {
    return fail("failed to create or validate the SQLite ledger");
  }
  auto application = pa::make_application_v8(
      std::get<ps::SQLiteLedgerV8>(std::move(ledger.result)));
  if (!std::holds_alternative<pa::ApplicationV8>(application.result)) {
  ...
  auto app = std::get<pa::ApplicationV8>(std::move(application.result));
