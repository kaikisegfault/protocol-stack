...
// version-eight meaning. **One conversion is real**: `InitChainRequest` carries
// a `protocol::v1::ChainId`, which is a tagged type and therefore distinct from
// version eight's `Octets32` even though both are thirty-two octets. It is
...
EncodedFrameResult encode_result(MessageKind kind, std::uint64_t request_id,
  ...
  if (std::holds_alternative<ApplicationError>(result)) {
    return encode_error_response_v8(kind, request_id,
                                    std::get<ApplicationError>(result));
  }
  return encode_success_response_v8(
      kind, request_id, SuccessResponseV8{std::get<Success>(std::move(result))});
}

EncodedFrameResult invalid_payload(const DecodedRequest& request) {
  return encode_error_response_v8(request.kind, request.request_id,
                                  ApplicationError::invalid_request);
}

v8::Octets32 chain_id_of(const protocol::v1::ChainId& tagged) {
  v8::Octets32 value{};
...
EncodedFrameResult dispatch_request_v8(ApplicationV8& application,
  ...
  switch (request.kind) {
    case MessageKind::info:
      ...
      return encode_result<ApplicationInfoV8>(request.kind, request.request_id,
                                              application.info());
    ...
    case MessageKind::finalize_block: {
      const auto* value = std::get_if<BlockRequest>(&request.payload);
      if (value == nullptr) return invalid_payload(request);
      return encode_result<FinalizedBlockV8>(
          request.kind, request.request_id,
    ...
    case MessageKind::commit:
      ...
      return encode_result<CommittedHeadV8>(request.kind, request.request_id,
                                            application.commit());
  }
  return invalid_payload(request);
}
