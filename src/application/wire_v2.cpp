...
//
// The primitive encoding is version one's and is unchanged: fixed-width
// big-endian integers, `i64` two's complement, `bytes32`, `blob` as
// `length:u32 || bytes`, `blob_list` as `count:u32 || blob[count]`, no padding,
// no trailing bytes. What this file holds is the version octet and the two
// payloads that gained a stamp; everything else reads the way version one's
// does, deliberately, so the two can be compared line for line.
...
// The transport must not admit a block the kernel would refuse on its input
// count alone. Version one's figure is the same number; this asserts that it
// still is, so the day the kernel's bound moves, this file stops compiling
// instead of quietly disagreeing with it.
static_assert(kMaximumBlockInputsV2 == kMaximumBlockInputs,
              "version two's raw-input bound is the kernel's");
...
std::variant<std::vector<Bytes>, WireError> read_transactions(Reader& reader) {
  const auto count = reader.u32();
  if (!count) return WireError::invalid_payload;
  if (*count > kMaximumBlockInputsV2) return WireError::resource_limit;
...
RequestResultV2 decode_payload(const Frame& frame) {
  ...
  switch (frame.header.kind) {
    ...
    case MessageKind::init_chain: {
      ...
      // **The stamp is read before the blob**, which is the whole of why it was
      // placed there: the frame is bounded before anything is allocated.
      const auto genesis_timestamp = reader.u64();
      auto app_state = reader.blob(kMaximumAppStateBytes);
      if (!raw_chain || !height || !genesis_timestamp ||
      ...
      payload = InitChainRequestV2{protocol::v1::ChainId{chain}, *height,
                                   *genesis_timestamp,
  ...
  return DecodedRequestV2{frame.header.kind, frame.header.request_id,
                          std::move(payload)};
}
...
HeaderResult decode_frame_header_v2(
  ...
  // **The one comparison that differs from version one's.** A version-one frame
  // arrives here and is refused as an unsupported version, at the header, on the
  // first frame — which is the refusal this version exists to make possible.
  if (read_u16(header.subspan(4, 2)) != kWireVersionV2) {
...
FrameResult decode_frame_v2(std::span<const std::uint8_t> bytes) {
  ...
  auto decoded = decode_frame_header_v2(bytes.first(kWireHeaderSize));
...
RequestResultV2 decode_request_frame_v2(std::span<const std::uint8_t> bytes) {
  auto decoded = decode_frame_v2(bytes);
...
EncodedFrameResult encode_frame_v2(const Frame& frame) {
  const auto raw_direction = static_cast<std::uint8_t>(frame.header.direction);
  const auto raw_kind = static_cast<std::uint8_t>(frame.header.kind);
  if (raw_direction > static_cast<std::uint8_t>(WireDirection::response) ||
  ...
  append_u16(encoded, kWireVersionV2);
