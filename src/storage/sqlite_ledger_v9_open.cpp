// How a version-nine store comes into existence, which is a different subject
...
SQLiteLedgerV9Result create_sqlite_ledger_v9(
    ...
    const v9::Genesis& genesis, v9::SignatureVerifier verify) {
  try {
...
SQLiteLedgerV9Result open_sqlite_ledger_v9(
  ...
  try {
    ...
    internal::validate_integrity_v9(resources->connection);
    internal::validate_schema_v9(resources->connection);
    internal::validate_stored_genesis_v9(resources->connection,
                                         bytes_view(trusted.canonical_bytes));
    auto durable = internal::read_durable_head_v9(resources->connection);
    auto payload = durable.snapshot;
