...
bool apply_cycle_assignment(Rebuild& rebuild, std::span<const std::uint8_t> key,
  ...
  const auto record = v8::decode_cycle_assignment_value(value);
  if (!record) return false;
  if (record->bitmap_bits > v8::kFounderSeatCapacity ||
      record->in_scope_count > v8::kFounderSeatCapacity) {
    return false;
  }
  ...
  const auto split = v8::split_permission(record->winner_count);
  if (record->share_per_winner_atomic != split.share[v8::kFounderOperatorChannel]) {
    return false;
  }
  rebuild.ledger.assignments.emplace(*window, v8::Bytes(value.begin(), value.end()));
  return true;
}
...
std::optional<std::uint64_t> derive_assigned_permissions(const v8::Ledger& ledger) {
  ...
  for (const auto& [window, value] : ledger.assignments) {
    ...
    if (contributing > v8::kMaxU64 - assigned) return std::nullopt;
