// The version-nine snapshot's framing: the prefix, the two ordered sections,
...
// merely to verify: the fixed fee, the ecosystem verifier key, and the dispute
// authority key.
//
// **The summary is version nine's, so it carries the head's timestamp.** That is
// the only change to the prefix, and it is eight octets wide at a fixed offset
// like every field beside it. A restored ledger that lost the stamp would keep
// producing blocks and keep satisfying C2 — the stale stamp is smaller than
// every later one — while committing roots that name a height the stamp does not
// belong to. The gates catch it because the root commits to it; the round-trip
// vector aims at it because a gate that happens to catch something is weaker
// evidence than a test that meant to.
//
...
using snapshot_v9::read_fixed;
using snapshot_v9::read_u16;
using snapshot_v9::read_u32;
using snapshot_v9::read_u64;
namespace v9 = protocol::v9;

// The magic is version one's. A snapshot family with one magic and a version
// field is what the version field is for: an earlier decoder reads these bytes,
// recognises the family, and reports an unsupported version rather than
// malformed input.
constexpr std::array<std::uint8_t, 4> kMagic{'P', 'S', 'S', 'N'};
constexpr std::uint16_t kVersion = 9;
// Version eight's 158 plus the head's timestamp. The decoder reads every prefix
// field at a literal offset, so this figure and those offsets are one fact
// written twice and the encoder checks it wrote exactly this many.
constexpr std::size_t kPrefixSize = 166;
constexpr std::size_t kAccountSize = v9::kAccountEntryBytes;
...
void append_length_prefixed(v9::Bytes& target,
...
struct Prefix {
  ...
  v9::Octets32 verifier_key{};
  v9::Octets32 dispute_authority_key{};
...
std::optional<Prefix> read_prefix(std::span<const std::uint8_t> payload) {
  ...
  // Immediately after the height, because the two are the head and a reader
  // looking for one should not have to step over the monetary fields to find
  // the other. `StateSummary` orders them the same way.
  const auto timestamp = read_u64(payload, 46);
  const auto supply_limit = read_u64(payload, 54);
  const auto total_supply = read_u64(payload, 62);
  const auto fee_pool = read_u64(payload, 70);
  const auto fixed_fee = read_u64(payload, 78);
  const auto verifier_key = read_fixed<32>(payload, 86);
  ...
  const auto dispute_authority_key = read_fixed<32>(payload, 118);
  const auto account_count = read_u64(payload, 150);
  const auto economy_count = read_u64(payload, 158);
  if (!chain_id || !height || !timestamp || !supply_limit || !total_supply ||
      !fee_pool || !fixed_fee || !verifier_key || !dispute_authority_key ||
      !account_count || !economy_count) {
    return std::nullopt;
  }
  prefix.summary.chain_id = *chain_id;
  prefix.summary.height = *height;
  prefix.summary.timestamp = *timestamp;
...
std::optional<std::vector<v9::AccountEntry>> read_accounts(
  ...
  for (std::size_t index = 0; index < count; ++index) {
    ...
    accounts.push_back(v9::AccountEntry{*account_id, *balance, *nonce});
    offset += kAccountSize;
  }
  return accounts;
}

std::optional<std::vector<v9::EconomyEntry>> read_economy(
...
SnapshotParametersV9 snapshot_parameters(const v9::Ledger& ledger) {
  return SnapshotParametersV9{ledger.chain_id, ledger.supply_limit,
                              ledger.fixed_fee, ledger.verifier_key,
                              ledger.dispute_authority_key};
}

SnapshotV9EncodeResult encode_snapshot_v9(const v9::Ledger& ledger) {
  const auto root = v9::ledger_state_root(ledger);
  if (!root) return SnapshotV9Error::invalid_state;
  const auto accounts = v9::account_entries(ledger);
  const auto economy = v9::economy_entries(ledger);

  std::size_t size = kFixedSize;
  if (!add_size(size, accounts.size() * kAccountSize)) {
    return SnapshotV9Error::size_overflow;
  }
  ...
  v9::Bytes payload;
  if (payload.max_size() < size) return SnapshotV9Error::size_overflow;
  ...
  append_u64(payload, ledger.timestamp);
  ...
  // prefix the same way and for the same reason — and version nine is the
  // version that added a field to both, which is exactly when the guard earns
  // its place.
  if (payload.size() != kPrefixSize) return SnapshotV9Error::size_overflow;
  ...
  if (payload.size() != size) return SnapshotV9Error::size_overflow;
  return EncodedSnapshotV9{std::move(payload), *root, digest};
}
...
SnapshotV9DecodeResult decode_snapshot_v9(
  ...
  if (parameters != expected_parameters) {
    return SnapshotV9Error::immutable_parameters_mismatch;
  }
  ...
  if (prefix->economy_count > body / 8) return SnapshotV9Error::size_overflow;
  ...
  if (offset != limit) return SnapshotV9Error::malformed;
  const auto state_root = read_fixed<32>(payload, limit);
  if (!state_root) return SnapshotV9Error::malformed;

  snapshot_v9::Rebuild rebuild;
  ...
  // The head's other half. It is restored here rather than in an entry decoder
  // because it is a summary field, exactly as the height is.
  rebuild.ledger.timestamp = prefix->summary.timestamp;
  ...
  for (const auto& account : *accounts) {
    rebuild.ledger.registry.accounts.emplace(
        account.account_id, v9::Account{account.balance, account.nonce});
  }
  for (const auto& entry : *economy) {
    if (!snapshot_v9::apply_entry(rebuild, entry)) {
      return SnapshotV9Error::invalid_state;
    }
  }
  if (!snapshot_v9::complete(rebuild)) return SnapshotV9Error::invalid_state;
  ...
  const auto restored_root = v9::ledger_state_root(rebuild.ledger);
  if (!restored_root || *restored_root != *state_root) {
    return SnapshotV9Error::state_root_mismatch;
  }
  ...
  const auto payload_root = v9::state_root(prefix->summary, *accounts, *economy);
  if (!payload_root || *payload_root != *state_root) {
    return SnapshotV9Error::payload_root_mismatch;
  }
  // Gate 3. A restore hands back a state some sequence of blocks could have
  // produced, or it hands back nothing. Under version nine this gate is where
  // the clock's own invariants run — the timestamp's range, the window-month
  // retention, and the one-open-month rule over the figures — beside the two
  // pool identities.
  if (!v9::conservation_failures(rebuild.ledger).empty()) {
    return SnapshotV9Error::not_conserved;
  }
  return DecodedSnapshotV9{std::move(rebuild.ledger), *state_root, *encoded_digest};
}
