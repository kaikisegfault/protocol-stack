...
bool apply_cycle_assignment(Rebuild& rebuild, std::span<const std::uint8_t> key,
  ...
  const auto record = v9::decode_cycle_assignment_value(value);
  if (!record) return false;
  if (record->bitmap_bits > v9::kFounderSeatCapacity ||
      record->in_scope_count > v9::kFounderSeatCapacity) {
    return false;
  }
  ...
  const auto split = v9::split_permission(record->winner_count);
  if (record->share_per_winner_atomic != split.share[v9::kFounderOperatorChannel]) {
    return false;
  }
  rebuild.ledger.assignments.emplace(*window, v9::Bytes(value.begin(), value.end()));
  return true;
}
...
std::optional<std::uint64_t> derive_assigned_permissions(const v9::Ledger& ledger) {
  ...
  for (const auto& [window, value] : ledger.assignments) {
    ...
    if (contributing > v9::kMaxU64 - assigned) return std::nullopt;
