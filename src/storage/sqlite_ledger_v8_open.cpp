// How a version-eight store comes into existence, which is a different subject
...
SQLiteLedgerV8Result create_sqlite_ledger_v8(
    ...
    const v8::Genesis& genesis, v8::SignatureVerifier verify) {
  try {
    ...
    try {
      internal::install_schema_v8(resources->connection,
    ...
    auto implementation = std::make_unique<SQLiteLedgerV8::Impl>(
    ...
    return SQLiteLedgerV8Result{
        std::variant<SQLiteLedgerV8, SQLiteLedgerV8Error>(
            std::in_place_type<SQLiteLedgerV8>,
            SQLiteLedgerV8(std::move(implementation))),
    };
    ...
    return error_result(SQLiteLedgerV8Error::storage_failure);
  }
}
...
SQLiteLedgerV8Result open_sqlite_ledger_v8(
  ...
  try {
    ...
    internal::validate_integrity_v8(resources->connection);
    internal::validate_schema_v8(resources->connection);
    internal::validate_stored_genesis_v8(resources->connection,
                                         bytes_view(trusted.canonical_bytes));
    auto durable = internal::read_durable_head_v8(resources->connection);
    auto payload = durable.snapshot;
