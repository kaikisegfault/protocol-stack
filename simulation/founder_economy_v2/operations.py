...
def carry_entries(before: int, after: int) -> list[JournalEntry]:
    """Journal a carry movement, emitting nothing when the carry is unchanged."""
    if after == before:
        return []
    if after > before:
        return [entry(CARRY_BUCKET, INCREASE, after - before)]
    return [entry(CARRY_BUCKET, DECREASE, before - after)]
...
def check_direct_issuable(
    state: State,
    channel_id: str,
    custody_key: str,
    amount: int,
) -> str | None:
    """Prove a direct mint fits its channel cap and its beneficiary bucket."""
    channel = state.channels[channel_id]
    reserved = checked_add(channel.issued_atomic, channel.outstanding_atomic)
    if reserved is None or checked_add(reserved, amount) is None:
        return "ARITHMETIC_OVERFLOW"
