...
Four of the five direct-mint caps are founder-directed lump sums with no
per-cycle derivation, so they are fixed by the contract table and reached here
only through the subtotal. The referral cap is different: it is the per-cycle
benefit times the complete seat-cycle population, and this stage proves it is
consumed with no remainder.

The stage is bound to a contract table rather than importing one, because every
accepted manifest version derives the same products from its own table.
"""
...
class Contract(Protocol):
    """The contract values this stage reads from a bound manifest version."""

    DIRECT_MINT: str
    REFERRAL_CHANNEL: str
    FOUNDER_CHANNEL_SUBTOTAL: int
    DIRECT_CHANNEL_SUBTOTAL: int
...
def check_derivations(root: dict[str, Any], contract: Contract) -> None:
    ...
    *leg_schedules, base_schedule, referral_schedule = products
    ...
    for leg, schedule in zip(root["base_permission"]["legs"], leg_schedules):
        if caps[leg["channel"]] != schedule:
            _fail("SUPPLY_MISMATCH", f"{leg['channel']} cap is not its full schedule")
    if caps[contract.REFERRAL_CHANNEL] != referral_schedule:
    ...
    founder = checked_sum(
        [
            ...
            if entry["issuance_kind"] != contract.DIRECT_MINT
        ]
    )
    direct = checked_sum(
        [
    ...
    if founder != base_schedule:
        _fail("SUPPLY_MISMATCH", "Founder Node subtotal is not the full base schedule")
    if founder != contract.FOUNDER_CHANNEL_SUBTOTAL:
        _fail("SUPPLY_MISMATCH", "Founder Node channel subtotal differs")
    if direct != contract.DIRECT_CHANNEL_SUBTOTAL:
