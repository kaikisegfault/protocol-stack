...
class Manifest:
    ...
    def maximum_supply_atomic(self) -> int:
        return c.MAXIMUM_SUPPLY_ATOMIC

    @property
    def channel_caps(self) -> dict[str, int]:
        return dict(c.CHANNEL_CAPS)
...
def load_manifest_file(path: str | Path) -> Manifest:
    ...
    return load_manifest_text(raw)


def load_manifest_text(raw: str) -> Manifest:
    return accept_manifest(parse_json(raw))
...
def accept_manifest(value: Any) -> Manifest:
...
def _check_shape(value: Any) -> None:
    ...
    for index, entry in enumerate(_array(root["channels"], len(c.CHANNELS), "channels")):
    ...
    _fields(_object(root["referral_permission"], "referral_permission"),
            {"channel", "beneficiary_kind", "amount_atomic"}, "referral_permission")
    _array(root["research_placeholders"], len(c.RESEARCH_PLACEHOLDERS),
           "research_placeholders")


def _check_schema(root: dict[str, Any]) -> None:
    if root["schema"] != c.MANIFEST_SCHEMA:
...
def _check_fixed_values(root: dict[str, Any]) -> None:
    ...
    _compare(denomination, expected_denomination, "denomination")
    _compare(
        root["seat_schedule"],
        {
    ...
    _compare(
        root["referral_permission"],
        {"channel": c.REFERRAL_CHANNEL,
         "beneficiary_kind": c.REFERRAL_BENEFICIARY_KIND,
