"""Version-three genesis bytes and chain identity.

The field table is version two's with a different schema version and a different
chain-ID label, so the prefix is still 110 bytes and the canonical object bound
still admits 21,843 account entries. What changes is that a version-three chain
has a different chain ID from any version-two chain with identical fields, which
is what makes the two alternative chains rather than one chain read two ways.

Three of version one's genesis requirements relax, and each is forced by founder
direction rather than chosen: a conforming chain opens with zero total supply
and zero accounts, and the fixed fee is permitted to be zero as the consequence.
"""
...
def version_two_chain_id(genesis: Genesis) -> bytes:
    """The same fields under version two's schema and label, for comparison."""
    return digest("protocol-stack:v2:chain-id", encode(genesis, schema_version=2))
...
def initial_economy_entries(verifier_key: bytes) -> dict[bytes, bytes]:
    ...
    Genesis writes nothing else: never a seat, a manager, a HUB registration, a
    referral balance, a custody entry, or a cycle assignment. Writing the fixed
    tables explicitly is what keeps an absent entry unambiguous, rather than
