"""The canonical version-three transaction envelope and its ten kind bodies.
...
Kinds 3 and 7 share a body length. A decoder therefore dispatches on the kind
byte, which is the rule version two stated for exactly this eventuality, and the
signing message differs because the kind byte sits at offset 6 inside every
signature preimage.
"""
...
def body_bytes(kind: int, body: dict[str, Any]) -> bytes:
    ...
    if kind == c.ADD_MANAGER:
        return (
            u32(body["seat_id"])
            + body["manager_account_id"]
            + body["biometric_signature"]
        )
    if kind == c.HUB_VERIFY:
        return body["hub_uniqueness_hash"] + body["biometric_signature"]
...
def _decode_body(kind: int, raw: bytes) -> dict[str, Any]:
    ...
    if kind == c.ADD_MANAGER:
        return {
            ...
            "biometric_signature": raw[36:100],
        }
    if kind == c.HUB_VERIFY:
        return {
            "hub_uniqueness_hash": raw[0:32],
            "biometric_signature": raw[32:96],
        }
    return {
        "channel_id": raw[0],
...
def _decode_set_mint_biometric(raw: bytes) -> dict[str, Any]:
    """Enabling protection needs only the manager's address signature.

    The signature field must therefore be 64 zero octets when enabling. Half of
    ADR 0033's asymmetry is checkable here, on the bytes alone; the other half —
    that a disabling signature verifies — reads the verifier key from state and
    ...
    if flag == 1 and signature != bytes(64):
        raise MalformedTransaction(
            "enabling carries no verifier signature and must encode 64 zero octets"
        )
    return {
        "seat_id": _read(raw, 0, 4),
        "enable": flag == 1,
