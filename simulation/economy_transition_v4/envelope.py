"""The canonical version-four transaction envelope and its twelve kind bodies.
...
Two pairs of kinds share a body length. A decoder therefore dispatches on the
kind byte, which is the rule version two stated for exactly this case; the kind
byte sits at offset 6 inside every signature preimage, so no signature crosses
a kind.
"""
...
def body_bytes(kind: int, body: dict[str, Any]) -> bytes:
    ...
    if kind in (c.ACTIVATE_SEAT, c.MINT_NODE_VERIFIED):
        return u32(body["seat_id"]) + body["hub_signature"]
    if kind == c.MINT_NODE:
    ...
    if kind == c.SET_MINT_BIOMETRIC:
        return (
            u32(body["seat_id"])
            + u8(1 if body["enable"] else 0)
            + body["hub_signature"]
        )
    if kind == c.ADD_MANAGER:
        return u32(body["seat_id"]) + body["manager_account_id"] + body["hub_signature"]
    if kind == c.HUB_REGISTER:
        return (
            body["hub_identity_hash"]
            + body["hub_public_key"]
            + body["verifier_signature"]
        )
    if kind in (c.HUB_ADD_ADDRESS, c.HUB_REMOVE_ADDRESS):
        return body["account_id"] + body["hub_signature"]
...
def _decode_body(kind: int, raw: bytes) -> dict[str, Any]:
    ...
    if kind == c.TRANSFER:
        return {"recipient_account_id": raw[0:32], "amount_atomic": _read(raw, 32, 8)}
    if kind == c.PURCHASE_SEAT:
    ...
    if kind == c.MINT_REFERRAL:
        return {}
    if kind == c.SET_MINT_BIOMETRIC:
        return _decode_set_mint_biometric(raw)
    if kind == c.ADD_MANAGER:
        return {
            "seat_id": _read(raw, 0, 4),
            "manager_account_id": raw[4:36],
            "hub_signature": raw[36:100],
        }
    if kind == c.HUB_REGISTER:
        return {
            "hub_identity_hash": raw[0:32],
            "hub_public_key": raw[32:64],
            "verifier_signature": raw[64:128],
        }
    if kind in (c.HUB_ADD_ADDRESS, c.HUB_REMOVE_ADDRESS):
        return {"account_id": raw[0:32], "hub_signature": raw[32:96]}
    return {
        "channel_id": raw[0],
...
def _decode_purchase(raw: bytes) -> dict[str, Any]:
    ...
    return {
        "seat_id": _read(raw, 0, 4),
        "purchaser_account_id": raw[4:36],
        "has_referrer": flag == 1,
        "referrer_account_id": referrer,
        "hub_signature": raw[69:133],
    }


def _decode_set_mint_biometric(raw: bytes) -> dict[str, Any]:
    """Enabling protection needs only a recorded manager's address signature.

    The signature field must therefore be 64 zero octets when enabling. Half of
    the asymmetry is checkable here, on the bytes alone; the other half — that a
    disabling signature verifies against the seat identity's key — reads state
    and belongs to execution.
    """
    flag = raw[4]
    if flag not in (0, 1):
        raise MalformedTransaction("enable is not a canonical bool")
    signature = raw[5:69]
    if flag == 1 and signature != bytes(64):
        raise MalformedTransaction(
            "enabling carries no HUB signature and must encode 64 zero octets"
        )
    return {
        "seat_id": _read(raw, 0, 4),
        "enable": flag == 1,
