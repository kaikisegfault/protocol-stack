...
def body_bytes(kind: int, body: dict[str, Any]) -> bytes:
    ...
    if kind == c.ACTIVATE_SEAT:
        return (
            ...
            + u32(body["referrer_seat_id"])
        )
    if kind in (c.EVALUATE_BASE_PERMISSION, c.ACCRUE_REFERRAL):
        return u32(body["seat_id"]) + u16(body["cycle_index"])
    if kind == c.EXERCISE_PERMISSION:
        winners = body["winners"]
        return (
            u32(body["seat_id"])
            + u16(body["cycle_index"])
            + u32(len(winners))
            + b"".join(u32(seat) for seat in winners)
        )
...
def expected_signed_length(kind: int, winner_count: int = 0) -> int:
    fixed = c.HEADER_BYTES + c.FIXED_BODY_BYTES[kind] + c.TRAILER_BYTES
    variable = 4 * winner_count if kind in c.VARIABLE_LENGTH_KINDS else 0
    return fixed + variable + c.SIGNATURE_BYTES
...
def decode_signed(raw: bytes) -> tuple[Transaction, bytes]:
    ...
    body_start = c.HEADER_BYTES
    body_end = len(raw) - c.TRAILER_BYTES - c.SIGNATURE_BYTES
    body = _decode_body(kind, raw[body_start:body_end])
    if len(raw) != expected_signed_length(kind, len(body.get("winners", ()))):
...
def _decode_body(kind: int, raw: bytes) -> dict[str, Any]:
    if kind not in c.VARIABLE_LENGTH_KINDS and len(raw) != c.FIXED_BODY_BYTES[kind]:
    ...
    if kind == c.ACTIVATE_SEAT:
        return _decode_activation(raw)
    if kind in (c.EVALUATE_BASE_PERMISSION, c.ACCRUE_REFERRAL):
        return {"seat_id": _read(raw, 0, 4), "cycle_index": _read(raw, 4, 2)}
    if kind == c.EXERCISE_PERMISSION:
        return _decode_exercise(raw)
    return {
        "channel_id": raw[0],
...
def _decode_exercise(raw: bytes) -> dict[str, Any]:
    if len(raw) < c.FIXED_BODY_BYTES[c.EXERCISE_PERMISSION]:
        raise MalformedTransaction("exercise body is shorter than its fixed part")
    count = _read(raw, 6, 4)
    if count > c.FOUNDER_SEAT_CAPACITY:
        raise MalformedTransaction("winner count above the seat capacity")
    if len(raw) != c.FIXED_BODY_BYTES[c.EXERCISE_PERMISSION] + 4 * count:
        raise MalformedTransaction("winner list length disagrees with its count")
    winners = tuple(_read(raw, 10 + 4 * index, 4) for index in range(count))
    for index in range(1, count):
        if winners[index] <= winners[index - 1]:
            raise MalformedTransaction("winner list is not strictly increasing")
    return {
        "seat_id": _read(raw, 0, 4),
        "cycle_index": _read(raw, 4, 2),
        "winners": winners,
    }
