// The version-eight snapshot decoder over arbitrary bytes.
//
...
const v8::Ledger& seed_ledger() {
  static const v8::Ledger ledger = [] {
    v8::Genesis genesis;
    genesis.network_id = 8;
...
const ps::SnapshotParametersV8& seed_parameters() {
  static const auto parameters = ps::snapshot_parameters(seed_ledger());
  return parameters;
}

const v8::Bytes& seed_payload() {
  static const v8::Bytes payload = [] {
    auto encoded = ps::encode_snapshot_v8(seed_ledger());
    require(std::holds_alternative<ps::EncodedSnapshotV8>(encoded));
    return std::get<ps::EncodedSnapshotV8>(encoded).payload;
  }();
  return payload;
}

void fuzz_payload(std::span<const std::uint8_t> input) {
  auto first = ps::decode_snapshot_v8(input, seed_parameters());
  auto second = ps::decode_snapshot_v8(input, seed_parameters());
  const bool restored = std::holds_alternative<ps::DecodedSnapshotV8>(first);
  require(restored == std::holds_alternative<ps::DecodedSnapshotV8>(second));
  if (!restored) {
    require(std::get<ps::SnapshotV8Error>(first) ==
            std::get<ps::SnapshotV8Error>(second));
    return;
  }
  const auto& decoded = std::get<ps::DecodedSnapshotV8>(first);
  // A restored state is one some sequence of blocks could have produced, and it
  // commits the root the payload claimed.
  require(v8::conservation_failures(decoded.ledger).empty());
  const auto root = v8::ledger_state_root(decoded.ledger);
  require(root.has_value() && *root == decoded.state_root);

  auto reencoded = ps::encode_snapshot_v8(decoded.ledger);
  require(std::holds_alternative<ps::EncodedSnapshotV8>(reencoded));
  const auto& payload = std::get<ps::EncodedSnapshotV8>(reencoded).payload;
...
void require_reseal_is_accepted() {
  ...
  reseal(payload);
  require(std::holds_alternative<ps::DecodedSnapshotV8>(
      ps::decode_snapshot_v8(payload, seed_parameters())));
}
