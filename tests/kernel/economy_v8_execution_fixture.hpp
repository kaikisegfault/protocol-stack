...
// The shared fixture behind the version-eight execution checks.
//
// The kernel must reproduce `test-vectors/economy-transition-v8-execution.txt`,
// which records four scenarios executed against a real state. Reproducing a
...
// `simulation/economy_transition_v8/trace.py`.
//
...
// blocks; `run_quiet_heights` executes it. The four scenarios run about 1.35
// million heights between their recorded blocks.
//
...
// A key the chain does not recognise, so a dispute signed by it is refused as
// `UNAUTHORIZED_DISPUTE` because the oracle holds no entry for it.
inline const Octets32 kForeignAuthorityKey = repeated(0xEE);
...
inline const Hash kAliceEscrow = v8::escrow_id(kAliceIdentity, 0);
inline const Hash kAliceSecondEscrow = v8::escrow_id(kAliceIdentity, 1);
inline const Hash kBobEscrow = v8::escrow_id(kBobIdentity, 0);
inline const Hash kCarolEscrow = v8::escrow_id(kCarolIdentity, 0);
...
// step must never audit it. A chain with no such seat cannot tell whether the
// issue step checks activation at all, because an unactivated seat's default
// activation height of zero puts it inside every window.
inline constexpr std::uint32_t kSpareSeat = 2;

// **Both seats activate near the end of window 0**, so window 1 is the first
// window either is in scope for and the trace runs exactly the windows it
// measures.
inline constexpr std::uint64_t kActivationHeight = v8::kCycleBlocks - 10;
...
inline constexpr std::uint64_t kDisputeHeight =
    (kMeasuredWindow + 1) * v8::kCycleBlocks + 100;

inline constexpr std::uint64_t kTransferAmountV8 = 1'000'000;
inline constexpr std::uint8_t kReasonCode = 3;
...
inline constexpr std::uint64_t kCollectionHeight = 40 * v8::kCycleBlocks;
...
inline constexpr std::uint64_t boundary_height(std::uint64_t window) {
  return (window + v8::kAssignmentLagWindows) * v8::kCycleBlocks;
}
// The first height of window 3 is where window 1's assignment is due.
inline constexpr std::uint64_t kAssignmentHeight = boundary_height(kMeasuredWindow);
...
class Signatures {
 public:
  Bytes sign(const Octets32& public_key, std::span<const std::uint8_t> message) {
    ...
    Bytes token(v8::kSignatureBytes, 0);
  ...
  v8::SignatureVerifier verifier() const {
...
struct Scenario {
  std::string name;
  v8::Ledger ledger;
  std::vector<v8::BlockOutcome> blocks;
  ...
  // the setup segment.
  std::vector<v8::BlockOutcome> audit_blocks;
...
v8::Ledger open_trace_ledger(const Octets32* chain_id = nullptr);
...
Bytes posture_input(Signatures& signatures, const v8::Ledger& ledger,
                    ...
                    std::uint64_t valid_until = kInheritedValidUntil);
// The four seat transactions, carried unchanged from version seven.
...
Bytes response_input(Signatures& signatures, const v8::Ledger& ledger,
                     ...
                     const Bytes* answer = nullptr);
// `authority_key` is a parameter so a scenario can offer a dispute signed by a
// key the chain does not recognise. The table holds no signature for a message
// it was never asked to sign, so the refusal is the absence of an entry rather
// than a comparison this fixture invents.
...
class Responder {
 public:
  Responder(Signatures& signatures, v8::Ledger& ledger, std::uint32_t seat_id,
            const Octets32& escrow, const Octets32& signer_key,
            bool silent = false)
      : signatures_(&signatures), ledger_(&ledger), seat_id_(seat_id),
        escrow_(escrow), signer_key_(signer_key), silent_(silent) {}
  ...
  //
  // **The nonce is not carried**, because it is not this object's to carry: it
  // is read from whichever ledger is being answered. A branch that filed six
  // disputes from the same escrow has a different nonce sequence from one that
  // filed none, and a responder holding its own counter would offer a stale
  // nonce to exactly one of the two.
  Responder fork(v8::Ledger& ledger) const {
    return Responder(*signatures_, ledger, seat_id_, escrow_, signer_key_,
                     silent_);
  }

  std::vector<std::uint64_t> challenged;
  std::vector<std::uint64_t> answered;
  std::set<std::uint32_t> slots() const;
...
// Run every height up to `target_height`, recording the blocks that were not
// quiet. `run_quiet_heights` is not a shorthand for anything: under version
// eight a block with no transactions still audits every in-scope seat.
std::uint64_t run_to(Scenario& scenario, const Signatures& signatures,
                     std::uint64_t target_height,
                     Responder* responder = nullptr);
// The one stretch of a version-eight chain where a block with no transactions
// really does change height and nothing else: before any seat is activated, the
// issue step selects nobody.
...
// Every recorded vector must have been consulted. A key in a section this test
// claims but never reads is the vacuous case `docs/engineering/verification.md`
// forbids, so nothing is exempt.
void verify_coverage(const pv::Values& values);
// The rejection conditions no recorded scenario reaches, and the 62 contract
// vectors a ledger is needed for. These are kept in their own translation units
