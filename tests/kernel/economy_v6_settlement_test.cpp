...
void verify_verified_user(const pv::Values& values) {
  ...
  pv::require(*daily == v6::kVerifiedUserDailyAtomic, "and the declared constant");
  pv::require(v6::kVerifiedUserChannel ==
                      expect_number(values, "verified_user.channel_id") &&
                  v6::kVerifiedUserPopulation ==
                      expect_number(values, "verified_user.population") &&
                  v6::kVerifiedUserCycles ==
  ...
  pv::require(v6::verified_user_remainder_at(730) ==
                  expect_number(values,
                                "verified_user.channel_cap_remainder_at_730_cycles"),
              "the remainder a 730-cycle period leaves");
  pv::require(v6::verified_user_remainder_at(v6::kVerifiedUserCycles) == 0,
  ...
  const Case cases[] = {
      {"verified_user.the_day_after_enrollment.", 6 * v6::kCycleBlocks},
      {"verified_user.at_the_cap.", 36 * v6::kCycleBlocks},
      {"verified_user.one_window_past_the_cap.", 37 * v6::kCycleBlocks},
      {"verified_user.ten_windows_past_the_cap.", 46 * v6::kCycleBlocks},
      {"verified_user.long_past_the_period.", 1'000 * v6::kCycleBlocks},
  };
  for (const auto& [prefix, height] : cases) {
    const auto collection = v6::verified_user_collection(enrolled, enrolled, height);
  ...
  const auto forfeiting =
      v6::verified_user_collection(enrolled, enrolled, 46 * v6::kCycleBlocks);
  ...
  pv::require(forfeiting.count == v6::kMintAccumulationCap,
              "a forfeiting mint collects exactly the cap");
  const auto later = v6::verified_user_collection(forfeiting.collectable_end, enrolled,
                                                  1'000 * v6::kCycleBlocks);
  ...
  std::uint64_t issued = v6::kVerifiedUserDailyAtomic;
  std::uint64_t mark = enrolled;
  const auto period_end = enrolled + v6::kVerifiedUserCycles - 1;
  while (true) {
    const auto target = std::min(mark + v6::kMintAccumulationCap, period_end);
    const auto collection =
        v6::verified_user_collection(mark, enrolled, (target + 1) * v6::kCycleBlocks);
  ...
  pv::require(v6::verified_user_collection(mark, enrolled, 5'000 * v6::kCycleBlocks)
                      .count ==
                  expect_number(values,
  ...
  pv::require(issued <= v6::kVerifiedUserChannelCapAtomic / v6::kVerifiedUserPopulation,
  ...
  std::string derived;
  for (const auto channel : v6::kDirectIssueChannels) {
  ...
  pv::require(std::find(v6::kDirectIssueChannels.begin(),
                        v6::kDirectIssueChannels.end(),
                        v6::kVerifiedUserChannel) == v6::kDirectIssueChannels.end(),
...
void verify_walk(const pv::Values& values) {
  pv::require(v6::kMintAccumulationCap ==
                      expect_number(values, "settlement.assignment_cap_windows") &&
                  v6::kAssignmentLagWindows ==
                      expect_number(values, "settlement.assignment_lag_windows"),
              "the cap and the lag");

  pv::require(v6::accrues(kCurrentMark + v6::kMintAccumulationCap, kCurrentMark),
              "a window within the cap accrues");
  pv::require(!v6::accrues(kCurrentMark + v6::kMintAccumulationCap + 1, kCurrentMark),
  ...
  const auto range = v6::walk_range(kCurrentMark, kCurrentMark + 100);
  ...
  pv::require(!v6::walk_range(kCurrentMark, kCurrentMark).has_value(),
              "a mark at the last assigned window walks nothing");
  pv::require(!v6::walk_range(kCurrentMark, std::nullopt).has_value(),
              "no window assigned yet walks nothing");
  pv::require(!v6::walk_range(kCurrentMark + 2, kCurrentMark).has_value(),
  ...
  pv::require(v6::walk_range(kCurrentMark, kCurrentMark + 1)->last_window ==
                  kCurrentMark + 1,
...
void verify_cycle_assignment(const pv::Values& values,
  ...
  const auto accrued_bits = v6::bitmap(accrued_seats, kBits);
  const auto winner_bits = v6::bitmap(winner_seats, kBits);
  ...
  pv::require(!v6::bitmap(beyond, kBits).has_value(),
              "a seat outside the bit count cannot be packed");

  const auto value = v6::cycle_assignment_value(assignment);
  ...
  for (const auto seat : winner_seats) {
    pv::require(v6::bit_is_set(decoded->winner_bitmap, seat), "a winner bit is set");
  }
  for (const auto seat : accrued_seats) {
    pv::require(v6::bit_is_set(decoded->accrued_bitmap, seat), "an accrued bit is set");
  }
  pv::require(!v6::bit_is_set(decoded->accrued_bitmap, 23),
              "a seat may win without accruing");
  pv::require(!v6::bit_is_set(decoded->winner_bitmap, 15),
              "a seat may accrue without winning");
  auto short_record = *value;
  short_record.pop_back();
  pv::require(!v6::decode_cycle_assignment_value(short_record).has_value(),
  ...
  v6::CycleAssignment outage;
  outage.in_scope_count = 6;
  outage.reallocated_count = 5;
  outage.bitmap_bits = kBits;
  outage.accrued_bitmap = *v6::bitmap({}, kBits);
  outage.winner_bitmap = *v6::bitmap({}, kBits);
  const auto outage_value = v6::cycle_assignment_value(outage);
  ...
  pv::require(hex(v6::cycle_assignment_key(kOutageWindow)) ==
