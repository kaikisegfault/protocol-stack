...
Octets32 first_escrow(const Octets32& identity) {
  return v7::escrow_id(identity, 0);
}
...
Bytes retired_kind_input(Signatures& signatures, const v7::Ledger& ledger) {
...
Bytes foreign_chain_input(Signatures& signatures, const v7::Ledger& ledger) {
...
Scenario millionth_scenario(Signatures& signatures) {
  ...
  auto& ledger = scenario.ledger;
  ledger.registry.enrolled_count = v7::kVerifiedUserPopulation - 1;
  const auto alice = first_escrow(kAliceIdentity);
...
Scenario recovery_scenario(Signatures& signatures) {
  ...
  revoke.signer_id = v7::signer_id(kMariaLostSignerKey);
  run(scenario, signatures,
      {{"maria_revokes_her_only_signer",
        build(signatures, ledger,
              static_cast<std::uint8_t>(v7::Kind::signer_revoke), kMariaKey, 1,
              revoke)}});
...
Scenario compatibility_scenario(Signatures& signatures,
  ...
  v7::Envelope accepted;
  accepted.kind = static_cast<std::uint8_t>(v7::Kind::native_transfer);
  accepted.chain_id = chain_id;
  accepted.scheme = v7::kSchemeSigner;
  accepted.authority_public_key = sender_key;
  accepted.nonce = kAcceptedNonce;
  v7::Body accepted_body;
  accepted_body.recipient_escrow_id = recipient;
  accepted_body.amount_atomic = kAcceptedAmount;
  accepted.body = v7::encode_body(accepted.kind, accepted_body);
  accepted.fee_limit = kAcceptedFeeLimit;
  accepted.valid_until_height = kAcceptedValidUntil;
  const auto unsigned_transaction = v7::encode_unsigned(accepted);
  signatures.adopt(sender_key, v7::signing_message(unsigned_transaction), signature);
  run(scenario, signatures,
      {{"the_accepted_transfer", v7::encode_signed(accepted, signature)}});
  ...
  run(scenario, signatures,
      {{"the_same_transfer_to_a_registered_recipient",
        build(signatures, ledger,
              static_cast<std::uint8_t>(v7::Kind::native_transfer), sender_key, 2,
