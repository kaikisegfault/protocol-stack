...
void verify_state_widths(const pv::Values& values) {
  ...
  for (const auto kind : assigned) {
    ...
    const auto key_width = v6::entry_key_bytes(kind);
    pv::require(key_width.has_value(), "entry kind is assigned");
    pv::require(*key_width == expect_size(values, prefix + "key_bytes"),
                "entry key width");
    const auto value_width = v6::entry_value_bytes(kind);
    if (kind == static_cast<std::uint8_t>(v6::Entry::cycle_assignment)) {
  ...
  for (const auto kind : retired) {
    pv::require(v6::is_retired_entry_kind(kind) && !v6::is_entry_kind(kind),
    ...
    pv::require(!v6::economy_root({{key, {}}}).has_value(),
                "a retired entry is refused by the tree");
  }
  pv::require(!v6::is_entry_kind(0) && !v6::is_entry_kind(17),
  ...
  pv::require(!v6::economy_root({{{200}, {}}}).has_value(), "an unknown entry kind");
  pv::require(!v6::economy_root({{v6::carry_key(0), {}}}).has_value(),
              "a value of the wrong width");
  pv::require(!v6::economy_root({{{7}, v6::carry_value(0)}}).has_value(),
              "a key of the wrong width");
  const v6::EconomyEntry carry{v6::carry_key(0), v6::carry_value(0)};
  pv::require(v6::economy_root({carry}).has_value(), "a well-formed entry hashes");
  pv::require(!v6::economy_root({carry, carry}).has_value(), "a duplicated key");
...
void verify_trees(const pv::Values& values, const pv::Values& version_three) {
  const auto empty = v6::economy_root({});
  ...
  const auto genesis_root = v6::economy_root(initial);
  ...
  for (const auto& entry : initial) {
    ...
    pv::require(kind != static_cast<std::uint8_t>(v6::Entry::seat) &&
                    kind != static_cast<std::uint8_t>(v6::Entry::hub_identity) &&
                    kind != static_cast<std::uint8_t>(v6::Entry::escrow) &&
                    kind != static_cast<std::uint8_t>(v6::Entry::signer),
  ...
  const auto populated_root = v6::economy_root(populated);
...
void verify_roots(const pv::Values& values, const pv::Values& primitives,
  ...
  pv::require(hex(v6::accounts_root(accounts)) ==
  ...
  pv::require(expect_text(values, "root.label") == v6::kStateRootLabel,
              "the state-root domain label");
  pv::require(expect_number(values, "root.schema_version") ==
                  v6::kStateRootSchemaVersion,
              "the state-root schema version");

  v6::StateSummary summary;
  ...
  const auto empty = v6::state_root(summary, accounts, {});
  ...
  const auto populated =
      v6::state_root(summary, accounts, populated_economy(version_three));
  ...
  const v6::EconomyEntry carry{v6::carry_key(0), v6::carry_value(0)};
  pv::require(!v6::state_root(summary, accounts, {carry, carry}).has_value(),
...
void verify_genesis(const pv::Values& values, const pv::Values& manifest) {
  ...
  pv::require(expect_number(values, "genesis.schema_version") ==
                  v6::kGenesisSchemaVersion,
              "the genesis schema version");
  pv::require(expect_text(values, "genesis.chain_id_label") == v6::kChainIdLabel,
              "the chain-ID domain label");
  const auto identifier = v6::chain_id(genesis);
  ...
  pv::require(!v6::encode_genesis(rejected).has_value(), "a zero supply limit");
  rejected = genesis;
  rejected.total_supply = 1;
  pv::require(!v6::encode_genesis(rejected).has_value(), "a nonzero total supply");
  rejected = genesis;
  rejected.initial_fee_pool = 1;
  pv::require(!v6::encode_genesis(rejected).has_value(), "a nonzero fee pool");
  rejected = genesis;
  rejected.account_count = 1;
  pv::require(!v6::encode_genesis(rejected).has_value(), "any account entry");
...
void verify_storage(const pv::Values& values) {
  auto entry = [](v6::Entry kind) {
    const auto number = static_cast<std::uint8_t>(kind);
    return *v6::entry_key_bytes(number) + *v6::entry_value_bytes(number);
  };
  const auto identity = entry(v6::Entry::hub_identity);
  const auto escrow = entry(v6::Entry::escrow) + v6::kAccountEntryBytes;
  const auto signer = entry(v6::Entry::signer);

  pv::require(entry(v6::Entry::seat) * v6::kFounderSeatCapacity ==
  ...
  pv::require(*v6::entry_key_bytes(
                  static_cast<std::uint8_t>(v6::Entry::cycle_assignment)) +
                      24 + 2 * v6::bitmap_bytes(v6::kFounderSeatCapacity) ==
