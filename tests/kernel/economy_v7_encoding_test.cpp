...
void verify_kind_table(const pv::Values& values) {
  ...
  for (const auto kind : assigned) {
    ...
    const auto body = v7::body_bytes(kind);
    const auto unsigned_size = v7::unsigned_bytes(kind);
    const auto signed_size = v7::signed_bytes(kind);
  ...
  pv::require(expect_size(values, "envelope.header_bytes") == v7::kHeaderBytes &&
                  expect_size(values, "envelope.trailer_bytes") == v7::kTrailerBytes &&
  ...
  for (const auto kind : retired) {
    pv::require(v7::is_retired_kind(kind), "the number is retired");
    pv::require(!v7::is_transaction_kind(kind), "a retired kind has no width");
  ...
  pv::require(!v7::is_transaction_kind(0) && !v7::is_transaction_kind(20),
...
void verify_kind_one_identity(const pv::Values& values,
  ...
  const auto unsigned_bytes = v7::encode_unsigned(envelope);
  ...
  const auto signed_bytes = v7::encode_signed(envelope, signature);
  ...
  pv::require(signed_bytes.size() ==
              ...
              "the signed width");
  pv::require(hex(v7::transaction_id(signed_bytes)) == primitives.at("tx_id"),
  ...
  pv::require(std::equal(accepted.begin(), accepted.begin() + v7::kHeaderBytes,
                         unsigned_bytes.begin()),
              "the header is the accepted transfer's first 80 octets");
  pv::require(std::equal(accepted.end() - v7::kTrailerBytes, accepted.end(),
                         unsigned_bytes.end() - v7::kTrailerBytes),
  ...
  pv::require(expect_text(values, "compatibility.unregistered_recipient_code") ==
                  *v7::result_code_name(static_cast<std::uint8_t>(
                      v7::Result::recipient_not_registered)),
  ...
  pv::require(v7::encode_unsigned(registered) != unsigned_bytes,
...
void verify_admission(const pv::Values& values) {
  const v7::Bytes signature(v7::kSignatureBytes, 0x11);
  const auto accepted =
      v7::encode_signed(transfer_envelope(kUnregisteredRecipient, 1'000'000),
                        signature);
  pv::require(v7::decode_signed(accepted).has_value(),
  ...
  pv::require(!v7::decode_signed(mutate(0, 'X')).has_value(), "wrong magic");
  pv::require(!v7::decode_signed(mutate(5, 2)).has_value(), "wrong schema version");
  pv::require(!v7::decode_signed(mutate(6, 0)).has_value(), "kind zero");
  pv::require(!v7::decode_signed(mutate(6, 20)).has_value(), "unknown kind");
  pv::require(!v7::decode_signed(mutate(6, 7)).has_value(), "retired kind 7");
  pv::require(!v7::decode_signed(mutate(6, 11)).has_value(), "retired kind 11");
  pv::require(!v7::decode_signed(mutate(39, 3)).has_value(), "unknown scheme");
  ...
  pv::require(!v7::decode_signed(mutate(39, v7::kSchemeIdentity)).has_value(),
              "a scheme this kind forbids");

  auto trailing = accepted;
  trailing.push_back(0);
  pv::require(!v7::decode_signed(trailing).has_value(), "trailing byte");
  auto truncated = accepted;
  truncated.pop_back();
  pv::require(!v7::decode_signed(truncated).has_value(), "truncated");
  ...
  auto purchase = body_envelope(v7::Kind::purchase_seat);
  pv::require(v7::decode_signed(v7::encode_signed(purchase, signature)).has_value(),
  ...
  auto posture = body_envelope(v7::Kind::set_security_posture);
  posture.body[0] = 1;
  for (std::size_t index = 0; index < 4; ++index) {
    posture.body[9 + index] =
        static_cast<std::uint8_t>(v7::kMaxExemptSlotMask >> (8 * (3 - index)));
  }
  pv::require(v7::decode_signed(v7::encode_signed(posture, signature)).has_value(),
  ...
  auto confirmed_mint = body_envelope(v7::Kind::mint_referral);
  std::fill(confirmed_mint.body.begin() + 32, confirmed_mint.body.end(), 0x44);
  pv::require(
      v7::decode_signed(v7::encode_signed(confirmed_mint, signature)).has_value(),
      "a mint carrying a confirmation is admitted, because admission reads no "
      "posture");
  pv::require(v7::decode_signed(
                  v7::encode_signed(body_envelope(v7::Kind::mint_referral), signature))
                  .has_value(),
  ...
  auto registration = body_envelope(v7::Kind::hub_register);
  pv::require(
      v7::decode_signed(v7::encode_signed(registration, signature)).has_value(),
  ...
  auto referral = body_envelope(v7::Kind::mint_referral, 0x22);
  const auto referral_raw = v7::encode_signed(referral, signature);
  auto across_schemes = referral_raw;
  across_schemes[6] = static_cast<std::uint8_t>(v7::Kind::signer_add);
  pv::require(!v7::decode_signed(across_schemes).has_value(),
              "a relabelling across schemes is refused");
  auto within_scheme = referral_raw;
  within_scheme[6] = static_cast<std::uint8_t>(v7::Kind::mint_verified_user);
  const auto decoded = v7::decode_signed(within_scheme);
  pv::require(decoded.has_value(), "a same-scheme relabelling decodes");
  pv::require(decoded->envelope.kind ==
                  static_cast<std::uint8_t>(v7::Kind::mint_verified_user),
              "and is read as the kind its byte names");
  pv::require(v7::signing_message(v7::encode_unsigned(decoded->envelope)) !=
                  v7::signing_message(v7::encode_unsigned(referral)),
...
void verify_hub_messages(const pv::Values& values) {
  ...
  const std::pair<const char*, v7::Bytes> built[] = {
      {"registration", v7::registration_message(chain, alice, kAliceKey,
                                                kAliceSignerKey, kValidUntil)},
      {"purchase", v7::purchase_message(chain, alice, 0, kValidUntil)},
      {"activation", v7::activation_message(chain, alice, 0, kValidUntil)},
      {"mint",
       v7::mint_message(chain, alice, static_cast<std::uint8_t>(v7::Kind::mint_node),
                        0, escrow, kValidUntil)},
      {"posture_relax",
       v7::posture_relax_message(chain, alice, escrow, v7::Posture{}, kValidUntil)},
      {"transfer_confirm", v7::transfer_confirm_message(chain, alice, escrow,
  ...
  const std::uint8_t mints[] = {
      static_cast<std::uint8_t>(v7::Kind::mint_node),
      static_cast<std::uint8_t>(v7::Kind::mint_referral),
      static_cast<std::uint8_t>(v7::Kind::mint_verified_user),
  };
  ...
  for (std::size_t left = 0; left < std::size(mints); ++left) {
    for (std::size_t right = left + 1; right < std::size(mints); ++right) {
      pv::require(v7::mint_message(chain, alice, mints[left], 0, escrow, kValidUntil) !=
                      v7::mint_message(chain, alice, mints[right], 0, escrow,
                                       kValidUntil),
  ...
  pv::require(v7::posture_relax_message(chain, alice, escrow, other, kValidUntil) !=
                  built[4].second,
              "a different posture is a different message");
  expect_true(values, "hub.posture_message_binds_the_exact_posture");
  pv::require(v7::mint_message(chain, kBobIdentity, mints[0], 0, escrow, kValidUntil) !=
                  built[3].second,
