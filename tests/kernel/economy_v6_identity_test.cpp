...
void verify_derivations(const pv::Values& values, const pv::Values& primitives) {
  pv::require(expect_text(values, "escrow.label") == v6::kEscrowLabel,
  ...
  const v6::Hash four[] = {kAliceFirstEscrow, kAliceSecondEscrow, kAliceThirdEscrow,
                           kBobEscrow};
  ...
  pv::require(hex(v6::signer_id(accepted_key)) == primitives.at("account_id"),
  ...
  pv::require(hex(v6::signer_id(kAliceSignerKey)) ==
...
void verify_posture(const pv::Values& values) {
  ...
  pv::require(expect_number(values, "posture.slots_per_window") == v6::kSlotsPerWindow &&
                  expect_number(values, "posture.slot_blocks") == v6::kSlotBlocks &&
                  expect_number(values, "posture.max_exempt_slot_mask") ==
                      v6::kMaxExemptSlotMask,
              "the accepted grid");

  pv::require(v6::slot_of(0) == expect_number(values, "posture.slot_of_height_0") &&
                  v6::slot_of(1'200) ==
  ...
  pv::require(v6::requires_confirmation(strict, 0, 0), "strict requires at zero");
  pv::require(v6::requires_confirmation(strict, kMaximumSupplyAtomic, 0),
              "strict requires at a large amount");
  v6::Posture off;
  off.requires_confirmation = false;
  pv::require(!v6::requires_confirmation(off, kMaximumSupplyAtomic, 0),
              "confirmation off never requires");
  v6::Posture minimum;
  minimum.min_amount_atomic = 100'000'000;
  pv::require(v6::requires_confirmation(minimum, 100'000'000, 0),
              "an amount at the minimum requires");
  pv::require(!v6::requires_confirmation(minimum, 99'999'999, 0),
              "an amount below the minimum does not require");
  v6::Posture exempt;
  exempt.exempt_slot_mask = 1U;
  pv::require(!v6::requires_confirmation(exempt, 1, 0), "an exempt slot does not");
  pv::require(v6::requires_confirmation(exempt, 1, v6::kSlotBlocks),
  ...
  // a protection.
  pv::require(v6::relaxes(strict, off), "turning confirmation off relaxes");
  pv::require(v6::relaxes(strict, minimum), "raising the minimum relaxes");
  pv::require(v6::relaxes(strict, exempt), "setting an exempt slot relaxes");
  v6::Posture mixed;
  mixed.min_amount_atomic = 100'000'000;
  mixed.exempt_slot_mask = 0;
  pv::require(v6::relaxes(exempt, mixed),
              "a change that weakens anything is a relaxation");
  pv::require(!v6::relaxes(off, strict), "turning confirmation on tightens");
  pv::require(!v6::relaxes(minimum, strict), "lowering the minimum tightens");
  pv::require(!v6::relaxes(exempt, strict), "clearing an exempt slot tightens");
  pv::require(!v6::relaxes(strict, strict) && strict == v6::Posture{},
...
void verify_result_codes(const pv::Values& values) {
  pv::require(v6::kResultCodeCount == expect_number(values, "codes.count"),
  ...
  pv::require(!v6::result_code_name(v6::kResultCodeCount).has_value(),
  ...
  pv::require(v6::kFrozenUnreachableCodes.size() ==
                  expect_size(values, "codes.unreachable_count"),
              "three frozen unreachable codes");
  for (const auto code : v6::kFrozenUnreachableCodes) {
    expect_true(values,
                "codes.code" + std::to_string(code) + "_is_frozen_and_unreachable");
    pv::require(v6::result_code_name(code).has_value(),
                "a frozen code keeps its number and its name");
  }
}

void verify_receipt(const pv::Values& values) {
  pv::require(v6::kReceiptVersion == expect_number(values, "receipt.version"),
              "the receipt version");

  // A successful transfer: it charges the fixed fee and issues nothing, because
  // a transfer moves units that already exist.
  v6::Receipt receipt;
  receipt.kind = static_cast<std::uint8_t>(v6::Kind::native_transfer);
  receipt.result_code = static_cast<std::uint8_t>(v6::Result::success);
  receipt.fee_charged = 7;
  const auto encoded = v6::encode_receipt(receipt);
  ...
  pv::require(encoded->size() == expect_size(values, "receipt.bytes"),
              "receipt width");
  const auto decoded = v6::decode_receipt(*encoded);
  ...
  for (const auto kind : non_issuing) {
    ...
    pv::require(!v6::encode_receipt(issuing).has_value(),
  ...
  v6::Receipt registration;
  registration.kind = static_cast<std::uint8_t>(v6::Kind::hub_register);
  registration.issued_atomic = v6::kVerifiedUserDailyAtomic;
  pv::require(v6::encode_receipt(registration).has_value(),
  ...
  v6::Receipt invalid = receipt;
  invalid.kind = 7;
  pv::require(!v6::encode_receipt(invalid).has_value(), "a retired kind is refused");
  invalid = receipt;
  invalid.result_code = v6::kResultCodeCount;
  pv::require(!v6::encode_receipt(invalid).has_value(), "an unknown result code");
  invalid = receipt;
  invalid.result_code = static_cast<std::uint8_t>(v6::Result::channel_cap);
  pv::require(!v6::encode_receipt(invalid).has_value(),
              "a failure that charged a fee");
  invalid.fee_charged = 0;
  invalid.kind = static_cast<std::uint8_t>(v6::Kind::mint_node);
  ...
  auto older = *encoded;
  older[5] = 4;
  pv::require(!v6::decode_receipt(older).has_value(), "a version-four receipt");
}
