...
inline v7::Octets32 repeated(std::uint8_t octet) {
  v7::Octets32 value{};
  value.fill(octet);
  return value;
}

inline v7::Octets32 ascending(std::uint8_t first) {
  v7::Octets32 value{};
...
inline v7::Octets32 from_hex(const std::string& hex) {
...
inline v7::Octets32 to_octets(const v7::Hash& value) {
  v7::Octets32 result{};
...
inline std::string hex(const v7::Hash& value) {
...
inline const v7::Octets32 kChainId = ascending(0);
inline const v7::Octets32 kUnregisteredRecipient = ascending(0x20);
inline const v7::Octets32 kAliceIdentity = repeated(0xA1);
inline const v7::Octets32 kAliceKey = repeated(0xA2);
inline const v7::Octets32 kAliceSignerKey = repeated(0xA3);
inline const v7::Octets32 kAliceSecondSignerKey = repeated(0xA4);
inline const v7::Octets32 kBobIdentity = repeated(0xB1);
inline const v7::Octets32 kBobKey = repeated(0xB2);
inline const v7::Octets32 kBobSignerKey = repeated(0xB3);
inline const v7::Octets32 kMariaIdentity = repeated(0xC1);
inline const v7::Octets32 kMariaKey = repeated(0xC2);
inline const v7::Octets32 kMariaNewSignerKey = repeated(0xC4);
inline const v7::Octets32 kVerifierKey = repeated(0x55);
inline const v7::Octets32 kDecisionId = repeated(0x11);
inline const v7::Octets32 kSingletonBeneficiary = repeated(0x00);

inline constexpr std::uint64_t kFeeLimit = 1'000;
inline constexpr std::uint64_t kValidUntil = 42;
inline constexpr std::uint64_t kRegistrationHeight = 5 * v7::kCycleBlocks;
...
inline constexpr std::uint64_t kSeatActivationHeight = 7 * v7::kCycleBlocks;
...
inline const v7::Hash kAliceFirstEscrow = v7::escrow_id(kAliceIdentity, 0);
inline const v7::Hash kAliceSecondEscrow = v7::escrow_id(kAliceIdentity, 1);
inline const v7::Hash kAliceThirdEscrow = v7::escrow_id(kAliceIdentity, 2);
inline const v7::Hash kBobEscrow = v7::escrow_id(kBobIdentity, 0);
inline const v7::Hash kMariaEscrow = v7::escrow_id(kMariaIdentity, 0);
...
inline v7::Envelope transfer_envelope(const v7::Octets32& recipient,
                                      std::uint64_t amount) {
...
inline v7::Envelope body_envelope(v7::Kind kind, std::uint8_t fill = 0) {
  const auto number = static_cast<std::uint8_t>(kind);
  const bool registration = kind == v7::Kind::hub_register;
  v7::Envelope envelope;
  envelope.kind = number;
  envelope.chain_id = kChainId;
  envelope.scheme = *v7::kind_scheme(number);
  envelope.nonce = registration ? 0 : 1;
  envelope.fee_limit = registration ? 0 : kFeeLimit;
  envelope.valid_until_height = kValidUntil;
  envelope.body.assign(*v7::body_bytes(number), fill);
  return envelope;
}
...
inline void append_registry(std::vector<v7::EconomyEntry>& entries) {
  auto identity = [&](const v7::Octets32& hash, const v7::Octets32& key,
    ...
    entries.push_back({v7::hub_identity_key(hash), v7::hub_identity_value(record)});
  };
  auto escrow = [&](const v7::Hash& identifier, const v7::Octets32& owner,
                    const v7::Posture& posture, std::uint32_t signers) {
    v7::EscrowRecord record;
    record.owner_hub_identity = owner;
    record.posture = posture;
    record.signer_count = signers;
    entries.push_back({v7::escrow_key(identifier), v7::escrow_value(record)});
  };
  auto signer = [&](const v7::Octets32& key, const v7::Hash& assigned) {
    entries.push_back({v7::signer_key(v7::signer_id(key)), v7::signer_value(assigned)});
  };
  auto enrollment = [&](const v7::Octets32& hash, std::uint64_t height) {
    v7::EnrollmentRecord record;
    record.enrolled_at_height = height;
    record.minted_through_window = v7::window_of_height(height);
    record.issued_atomic = v7::kVerifiedUserDailyAtomic;
    entries.push_back({v7::verified_user_key(hash), v7::verified_user_value(record)});
  };
  ...
  v7::Posture relaxed_minimum;
  relaxed_minimum.min_amount_atomic = 100'000'000;
  escrow(kAliceFirstEscrow, kAliceIdentity, v7::Posture{}, 2);
  ...
  escrow(kBobEscrow, kBobIdentity, v7::Posture{}, 1);
  escrow(kMariaEscrow, kMariaIdentity, v7::Posture{}, 1);
...
inline std::vector<v7::EconomyEntry> populated_economy(
  ...
  std::erase_if(entries, [](const v7::EconomyEntry& entry) {
    return entry.key == v7::verified_user_counter_key();
  });
  append_registry(entries);
  entries.push_back(
      {v7::verified_user_counter_key(), v7::verified_user_counter_value(3)});

  v7::SeatRecord activated;
  ...
  entries.push_back({v7::seat_key(0), v7::seat_value(activated)});

  v7::SeatRecord purchased;
  purchased.hub_identity_hash = kAliceIdentity;
  entries.push_back({v7::seat_key(v7::kMaxSeatId), v7::seat_value(purchased)});

  entries.push_back({v7::referral_balance_key(kBobIdentity),
                     v7::referral_balance_value(kReferralLegAtomic * 3,
                                                kReferralLegAtomic, kCurrentMark)});
  entries.push_back({v7::direct_decision_key(kDecisionId), {}});
  entries.push_back({v7::typed_custody_key(1, kSingletonBeneficiary),
                     v7::typed_custody_value(17'100'000'000)});
  ...
  entries.push_back({v7::cycle_assignment_key(kCycleWindow),
                     pv::hex_decode(version_three.at("cycle.assignment_value_hex"))});
  entries.push_back({v7::cycle_assignment_key(kOutageWindow),
