// The version-eight transport: request frames in, response frames out.
//
...
using Bytes = v8::Bytes;

// Pinned to its literal for the reason `application_v8_test.cpp` records: the
// check below compares what the wire carried against the constant, which is a
// claim about the encoding and not about the number.
...
Response exchange(pa::ApplicationV8& application, pa::MessageKind kind,
  ...
  auto encoded = pa::dispatch_request_v8(
...
Response require_ok(pa::ApplicationV8& application, pa::MessageKind kind,
                    std::uint64_t request_id, const Bytes& payload,
                    const std::string& subject) {
...
Bytes init_chain_payload(const v8::Octets32& chain_id, std::uint64_t height,
...
pa::ApplicationV8 open_application(const std::filesystem::path& path,
  ...
  pv::require(std::holds_alternative<pa::ApplicationV8>(made.result),
...
void check_pipeline(const pv::Values& values,
  ...
  auto init = require_ok(application, pa::MessageKind::init_chain, request_id++,
                         init_chain_payload(trace_chain_id(), 1, kAppStateV8),
                         "init_chain");
  ...
  for (std::size_t index = 0; index < kContiguousBlocks; ++index) {
    ...
    for (std::uint32_t result = 0; result < result_count; ++result) {
      ...
      // Every admitted input carries a version-eight receipt whose own result
    ...
    pv::require(info_reader.u64() == pa::kApplicationProtocolVersionV8,
...
void check_read_only_operations(const std::filesystem::path& directory) {
  ...
  pv::require(refused_reader.u32() ==
                  pa::application_code(
                      v8::AdmissionError::malformed_transaction),
...
void check_encoder_refusals() {
  pa::FinalizedBlockV8 block;
  block.transaction_results.push_back(
      pa::TransactionResult{pa::application_code(v8::Result::success),
                            Bytes(v8::kReceiptBytes, 0x00)});
  pv::require(
      std::holds_alternative<pa::WireError>(pa::encode_success_response_v8(
          pa::MessageKind::finalize_block, 1, pa::SuccessResponseV8{block})),
      "a receipt without the version-eight prefix must not be written");

  pa::FinalizedBlockV8 mismatched;
  v8::Receipt receipt;
  receipt.kind = 1;
  receipt.result_code = static_cast<std::uint8_t>(v8::Result::unauthorized);
  auto encoded = v8::encode_receipt(receipt);
  pv::require(encoded.has_value(), "the probe receipt encodes");
  mismatched.transaction_results.push_back(pa::TransactionResult{
      pa::application_code(v8::Result::success), *encoded});
  pv::require(
      std::holds_alternative<pa::WireError>(pa::encode_success_response_v8(
          pa::MessageKind::finalize_block, 1,
          pa::SuccessResponseV8{mismatched})),
  ...
  pv::require(
      std::holds_alternative<pa::WireError>(pa::encode_success_response_v8(
          ...
          pa::SuccessResponseV8{pa::TransactionResult{0, Bytes{1, 2, 3}}})),
  ...
  pv::require(
      std::holds_alternative<pa::WireError>(pa::encode_success_response_v8(
          pa::MessageKind::commit, 1, pa::SuccessResponseV8{true})),
...
// the version-eight overload of `serve_connection`. The connection loop itself
// is version one's, shared rather than copied, so what this adds is that the
// decoded request reaches the *version-eight* dispatcher and its answer comes
...
int main(int argc, char** argv) {
  try {
    pv::require(argc == 3, "usage: application_transport_v8_tests VECTORS DIR");
    ...
    std::cout << "C++ version-eight transport: passed\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "C++ version-eight transport: failed: " << error.what() << '\n';
    return 1;
  }
}
