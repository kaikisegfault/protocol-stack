// The version-eight application layer, driven through the recorded execution
...
#include "../storage/sqlite_ledger_v8_fixture.hpp"
...
using namespace sqlite_ledger_v8_tests;

constexpr std::uint8_t kAppStateV8[] = {
    '"', 'p', 'r', 'o', 't', 'o', 'c', 'o', 'l', '-',
    's', 't', 'a', 'c', 'k', '-', 'v', '8', '"',
};

// **The protocol version is pinned to its literal, not to the constant.**
// Comparing `info().application_version` against
// `kApplicationProtocolVersionV8` — which the checks below do — is a claim that
// the value reaches the caller, and no claim at all about *which* value it is:
// it passes unchanged if the constant is still version seven's 7. This is the
// figure-that-moves-with-a-version rule M3.13q arrived at, applied to the one
// figure this layer moves that nothing else would catch.
static_assert(pa::kApplicationProtocolVersionV8 == 8);
...
static_assert(pa::application_code(v8::AdmissionError::malformed_transaction) ==
              1);
static_assert(pa::application_code(v8::AdmissionError::wrong_chain) == 2);
static_assert(pa::application_code(v8::AdmissionError::invalid_signature) == 3);
static_assert(pa::application_code(v8::Result::success) == 0);
static_assert(pa::application_code(v8::Result::zero_amount) == 257);
static_assert(pa::application_code(v8::Result::unauthorized) == 265);
static_assert(pa::application_code(v8::Result::not_enrolled) == 288);
...
v8::Octets32 trace_chain_id() {
  const auto identity = v8::chain_id(fixture::trace_genesis());
...
pa::ApplicationV8 open_application(const std::filesystem::path& path,
                                   bool create) {
  const auto genesis = fixture::trace_genesis();
  auto store = require_store(
      create ? ps::create_sqlite_ledger_v8(path, genesis, trace_verifier())
             : ps::open_sqlite_ledger_v8(path, genesis, trace_verifier()),
      create ? "creating the store" : "reopening the store");
  auto made = pa::make_application_v8(std::move(store));
  ...
  return std::get<pa::ApplicationV8>(std::move(made.result));
}

// Drive one recorded block all the way through, and compare it against the
// vectors rather than against the application.
void drive_block(pa::ApplicationV8& application, const pv::Values& values,
                 std::size_t index) {
  ...
  auto finalized = require_value<pa::FinalizeBlockResultV8, pa::FinalizedBlockV8>(
  ...
  // hide.
  auto again = require_value<pa::FinalizeBlockResultV8, pa::FinalizedBlockV8>(
  ...
  auto committed = require_value<pa::CommitResultV8, pa::CommittedHeadV8>(
  ...
  auto info = require_value<pa::InfoResultV8, pa::ApplicationInfoV8>(
      application.info(), label + ": info");
  pv::require(info.application_version == pa::kApplicationProtocolVersionV8,
...
void check_pipeline_across_restarts(const pv::Values& values,
  ...
  const auto path = directory / "application.db";
  {
    ...
    auto root = require_value<pa::InitChainResultV8, v8::Hash>(
...
void check_rejected_input_is_invisible(const pv::Values& values,
  ...
  (void)application.init_chain(trace_chain_id(), 1, kAppStateV8);
  ...
  inputs.push_back(v8::Bytes(8, 0x00));
  ...
  pv::require(refused.code ==
                  pa::application_code(v8::AdmissionError::malformed_transaction),
...
void check_init_chain_refusals(const std::filesystem::path& directory) {
  const auto chain_id = trace_chain_id();
  {
    ...
    auto other = chain_id;
    other[0] ^= 0x01;
    require_error(application.init_chain(other, 1, kAppStateV8),
    ...
    require_error(application.init_chain(chain_id, 1, kAppStateV8),
                  pa::ApplicationError::sequence_failure,
  ...
  }
  {
    ...
    require_error(application.init_chain(chain_id, 2, kAppStateV8),
                  pa::ApplicationError::invalid_request,
  ...
  {
    // **The neighbour that matters.** Version one's app state is a case version
    // seven's suite already had; version *seven's* is the one a stale
    // deployment would actually still be sending, and it is the only thing that
    // distinguishes a moved app-state constant from an unmoved one. Without it
    // the whole suite passes with `kExpectedAppStateV8` still naming version
    // seven.
    auto application = open_application(directory / "init-state-v7.db", true);
    constexpr std::uint8_t kVersionSevenState[] = {
        '"', 'p', 'r', 'o', 't', 'o', 'c', 'o', 'l', '-',
        's', 't', 'a', 'c', 'k', '-', 'v', '7', '"',
    };
    require_error(application.init_chain(chain_id, 1, kVersionSevenState),
                  pa::ApplicationError::invalid_request,
                  "init_chain with version seven's app state");
  }
}
...
void check_sequence_refusals(const std::filesystem::path& directory) {
  ...
  const auto& scenario = carried_scenario();
  {
    ...
    (void)application.init_chain(chain_id, 1, kAppStateV8);
...
void check_proposal_refusals(const std::filesystem::path& directory) {
  ...
  const std::vector<v8::Bytes> too_many(pa::kMaximumBlockInputsV8 + 1,
                                        v8::Bytes{});
  ...
  auto prepared =
      require_value<pa::PrepareProposalResultV8, pa::PreparedProposal>(
...
void check_transaction_admission(const std::filesystem::path& directory) {
  ...
  auto accepted =
      require_value<pa::TransactionCheckResultV8, pa::TransactionResult>(
...
int main(int argc, char** argv) {
  try {
    pv::require(argc == 3, "usage: application_v8_tests VECTORS DIR");
    ...
    std::cout << "C++ version-eight application: passed\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "C++ version-eight application: failed: " << error.what()
              << '\n';
    return 1;
  }
}
