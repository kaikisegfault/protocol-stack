...
// A version-nine snapshot payload taken apart, so a test can put exactly one
...
// `encode_snapshot_v9`'s for the same ledger. A rebuilder that drifted would
...
//
// **`timestamp` sits beside `height` here for the reason it does in the
// payload**: they are the head, and a test that wants to break one should not
// have to count past the monetary fields to find it.
...
struct Payload {
  ...
  std::vector<v9::EconomyEntry> economy;
  v9::Hash state_root{};
...
std::vector<v9::EconomyEntry>::iterator find_entry(Payload& payload,
                                                   v9::Entry kind);
// Remove every entry of a kind, for the cases about a fixed entry's absence.
...
// The recorded scenarios and the refusals, each its own translation unit.
