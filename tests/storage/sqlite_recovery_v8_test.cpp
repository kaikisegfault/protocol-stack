// The version-eight store's write path under injected faults, and what it
// recovers from.
//
...
// honest refusal. **None of that is version-specific**, so version eight
// inherits it whole and this file re-establishes it against a version-eight
...
// `test-vectors/economy-transition-v8-execution.txt`, so a store that persisted
...
ps::SQLiteLedgerV8 open_store(const std::filesystem::path& path, bool create) {
...
std::uint64_t durable_height(const std::filesystem::path& path) {
  ...
  return std::get<ps::LedgerHeadV8>(std::move(head)).ledger.height;
}
...
void verify_rolled_back_faults(const pv::Values& values,
  ...
  for (const auto& single : cases) {
    ...
    const auto& scenario = carried_scenario();
    {
      ...
      mode = Mode::none;
      pv::require(std::holds_alternative<ps::SQLiteLedgerV8Error>(applied),
                  std::string("a fault ") + single.name +
                      " must refuse the block");
      pv::require(std::get<ps::SQLiteLedgerV8Error>(applied) ==
                      ps::SQLiteLedgerV8Error::storage_failure,
                  std::string("a fault ") + single.name +
...
void verify_unrecoverable_commit_stays_poisoned(
  ...
  auto head = store.read_head();
  pv::require(std::holds_alternative<ps::SQLiteLedgerV8Error>(head) &&
                  std::get<ps::SQLiteLedgerV8Error>(head) ==
                      ps::SQLiteLedgerV8Error::storage_failure,
              "a store that could not recover must refuse to read its head");
  auto snapshot = store.create_snapshot();
  pv::require(std::holds_alternative<ps::SQLiteLedgerV8Error>(snapshot),
              "a poisoned store must refuse to hand out a payload");
  auto again = store.apply_block(1, scenario.block_inputs[0]);
  pv::require(std::holds_alternative<ps::SQLiteLedgerV8Error>(again),
...
int main(int argc, char** argv) {
  try {
    ...
    pv::require(argc == 3, "usage: storage_sqlite_recovery_v8_tests VECTORS DIR");
    ...
    std::filesystem::remove_all(directory);
    std::cout << "C++ version-eight store recovery: passed\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "C++ version-eight store recovery: failed: " << error.what()
              << '\n';
    return 1;
  }
}
