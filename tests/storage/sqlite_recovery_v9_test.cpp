// The version-nine store's write path under injected faults, and what it
// recovers from.
//
// ADR 0057's update settled the contract for version seven — everything before
// the commit rolls back and leaves the store usable, only the commit can leave
// a head this process cannot name, and recovery is allowed to fail into an
// honest refusal. **None of that is version-specific**, so version nine
// inherits it whole and this file re-establishes it against a version-nine
// chain rather than restating it. The property it proves is the one requirement
// 13 actually asks for — **"through restart *and recovery*"**:
//
...
// `test-vectors/economy-transition-v9-execution.txt`, so a store that persisted
// a subtly different state would fail here rather than agree with itself. **The
// head it compares is two scalars and a root under version nine**, so every
// place version eight compared a height, this compares the height and the stamp
// together: a recovery that found the right height with the predecessor's stamp
// would be the defect this version could hide.
...
ps::SQLiteLedgerV9 open_store(const std::filesystem::path& path, bool create) {
...
v9::Head durable_head(const std::filesystem::path& path) {
  ...
  return v9::Head{value.ledger.height, value.ledger.timestamp};
}

// The head a committed block leaves, from the vectors rather than the trace.
v9::Head recorded_head(const pv::Values& values, std::size_t index) {
  const auto label = block_label(index);
  return v9::Head{recorded_number(values, label + ".height"),
                  recorded_number(values, label + ".timestamp")};
}

// The genesis head, which is where every refused block must leave the store.
const v9::Head kGenesisHead{0, fixture::kGenesisMillis};

// Offer the run's first block, at the stamp it was recorded under.
ps::SQLiteV9BlockResult offer_first_block(ps::SQLiteLedgerV9& store) {
  const auto& run = restart_run();
  return store.apply_block(1, run.blocks[0].timestamp, run.block_inputs[0]);
}
...
void verify_rolled_back_faults(const pv::Values& values,
  ...
  for (const auto& single : cases) {
    const auto path = directory / ("rollback" + std::to_string(index++) + ".db");
    {
      ...
      mode = single.mode;
      auto applied = offer_first_block(store);
      mode = Mode::none;
      pv::require(std::holds_alternative<ps::SQLiteLedgerV9Error>(applied),
                  std::string("a fault ") + single.name +
                      " must refuse the block");
      pv::require(std::get<ps::SQLiteLedgerV9Error>(applied) ==
                      ps::SQLiteLedgerV9Error::storage_failure,
                  std::string("a fault ") + single.name +
      ...
      const auto live = std::get<ps::LedgerHeadV9>(std::move(head)).ledger;
      pv::require(v9::Head{live.height, live.timestamp} == kGenesisHead,
                  std::string("a fault ") + single.name +
    ...
    pv::require(durable_head(path) == recorded_head(values, 0),
                std::string("the block after a fault ") + single.name +
                    " did not become durable");
    pv::require(durable_root(path) ==
                    recorded(values, "restart.block0.resulting_state_root"),
...
void verify_commit_failure_recovers(const pv::Values& values,
  ...
  const auto path = directory / "commit-failure.db";
  {
    ...
    const auto value = std::get<ps::LedgerHeadV9>(std::move(head));
    pv::require(v9::Head{value.ledger.height, value.ledger.timestamp} ==
                    kGenesisHead,
  ...
  pv::require(durable_head(path) == recorded_head(values, 0),
              "the recovered store's durable head is not the recorded one");
  pv::require(durable_root(path) ==
                  recorded(values, "restart.block0.resulting_state_root"),
...
void verify_unrecoverable_commit_stays_poisoned(
  ...
  auto head = store.read_head();
  pv::require(std::holds_alternative<ps::SQLiteLedgerV9Error>(head) &&
                  std::get<ps::SQLiteLedgerV9Error>(head) ==
                      ps::SQLiteLedgerV9Error::storage_failure,
              "a store that could not recover must refuse to read its head");
  auto snapshot = store.create_snapshot();
  pv::require(std::holds_alternative<ps::SQLiteLedgerV9Error>(snapshot),
              "a poisoned store must refuse to hand out a payload");
  auto again = offer_first_block(store);
  pv::require(std::holds_alternative<ps::SQLiteLedgerV9Error>(again),
...
void verify_terminations(const pv::Values& values,
  ...
  for (const auto& single : cases) {
    ...
    pv::require(durable_head(path) == recorded_head(values, 1),
                std::string("the chain after a termination ") + single.name +
                    " is not at the recorded head");
    pv::require(durable_root(path) ==
                    recorded(values, "restart.block1.resulting_state_root"),
                std::string("the chain after a termination ") + single.name +
                    " diverged");
  }
}
...
int main(int argc, char** argv) {
  try {
    ...
    pv::require(argc == 3, "usage: storage_sqlite_recovery_v9_tests VECTORS DIR");
    ...
    std::filesystem::remove_all(directory);
    std::cout << "C++ version-nine store recovery: passed\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "C++ version-nine store recovery: failed: " << error.what()
              << '\n';
    return 1;
  }
}
