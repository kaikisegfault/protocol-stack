...
// The version-eight client sends version one's frames. Only the response it
...
func TestVersionEightClientSendsVersionOnesRequest(t *testing.T) {
	clientConnection, serverConnection := net.Pipe()
	defer serverConnection.Close()
	client := newClientV8(newClient(clientConnection))
	defer client.Close()
	...
	if block.StateRoot != root || block.BlockID != blockID ||
		...
		len(block.TransactionResults[0].Data) != receiptBytesV8 ||
...
// so a version-eight client answers them without a second copy of anything.
func TestVersionEightClientSharesTheOtherOperations(t *testing.T) {
	clientConnection, serverConnection := net.Pipe()
	defer serverConnection.Close()
	client := newClientV8(newClient(clientConnection))
	defer client.Close()
	...
	<-serverDone
	if info.ApplicationVersion != 8 || info.Height != 4 ||
		info.StateRoot != root {
		t.Fatalf("unexpected Info: %#v", info)
	}
}

// A protocol failure in the version-eight response is terminal for the whole
// connection, the same as version one's, because the latch is the embedded
// client's and there is only one of it.
func TestVersionEightProtocolFailureIsTerminal(t *testing.T) {
	clientConnection, serverConnection := net.Pipe()
	defer serverConnection.Close()
	client := newClientV8(newClient(clientConnection))
	...
	go func() {
		...
		// version-eight decoder reads is the identifier's leading octets.
		body := make([]byte, 32)
